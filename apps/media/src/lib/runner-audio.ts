...
export async function generateAudioBatch(
  opts: { limit?: number; dryRun?: boolean; lang?: string } = {}
): Promise<BatchResult> {
  ...
  const concurrency = Math.max(
    1,
    Number(process.env.MEDIA_TTS_CONCURRENCY) || 5
  )
  const checkpointEvery = Math.max(
    1,
    Number(process.env.MEDIA_TTS_CHECKPOINT) || 200
  )
  ...
  const storageKeys = client
    ? await existingAudioKeys(cfg, client)
    : new Set<string>()
  ...
  const recordKey = (e: ManifestEntry, gender: string) =>
    `${e.key}__${e.locale}__${gender}`
  ...
  const synthWithRetry = async (task: Task): Promise<Buffer> => {
    try {
      return await synthesizeAudioWithRotation(
        cfg,
        task.entry.text,
        task.entry.locale as MediaLocale,
        task.gender
      )
    } catch (err) {
      if (isTransient(err)) {
    ...
    try {
      ...
      if (storageKeys.has(task.objectKey)) {
        ...
        newRecords.push({
          ...
          audioRef:
            existing?.audioRef ??
            (task.owner !== task.key ? task.owner : undefined),
        })
        return
      }
      ...
      try {
        ...
        errors++
        console.error(
          `  ✗ ${task.key}: ${err instanceof Error ? err.message : err}`
        )
      }
    } finally {
  ...
  const worker = async () => {
    while (true) {
      ...
      if (!opts.dryRun && generated + copied > before)
