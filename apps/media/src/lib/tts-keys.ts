  ...
  if (dbConfigured()) {
    try {
      if (!poolCache || Date.now() - poolCache.fetchedAt > POOL_CACHE_TTL_MS) {
        poolCache = {
          keys: await fetchPoolRows<TtsStoredKey>(POOL_TABLE),
          fetchedAt: Date.now(),
        }
      }
      ...
      console.warn(
        `  ⚠ No enabled ${provider} TTS keys in the pool (all in cooldown?) — falling back to env`
      )
    } catch (err) {
      console.warn(
        `  ⚠ tts_api_keys fetch failed (${errMessage(err)}) — falling back to env keys`
      )
    }
  }
 ...
 */
export async function reportTtsKeyStatus(
  key: TtsKeySpec,
  err?: unknown
): Promise<void> {
  if (key.name === "env" || !dbConfigured()) return
  const patch: Partial<TtsStoredKey> = {
...
export async function synthesizeAudioWithRotation(
  ...
  if (cfg.ttsEngine === "edge")
    return synthesizeAudioWithKey(cfg, text, locale, gender)
  ...
  throw (
    lastErr ?? new Error(`TTS synthesis failed: no keys left for ${provider}`)
  )
}
...
export async function patchTtsStoredKey(
  ...
  patch: Partial<
    Pick<
      TtsStoredKey,
      "api_key" | "region" | "enabled" | "cooldown_until" | "last_error"
    >
  >
): Promise<void> {
  if (!dbConfigured())
