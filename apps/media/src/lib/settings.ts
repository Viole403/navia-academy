...
export const MEDIA_SETTING_KEYS = ["image_provider", "tts_engine", "vision_provider"] as const;
...
export async function readMediaSettings(): Promise<MediaSettingOverrides> {
  ...
  const res = await fetch(`${supabaseUrl()}/rest/v1/${TABLE}?select=key,value`, { headers: authHeaders() });
  ...
  for (const r of rows) {
    if (r.key === "image_provider") overrides.imageProvider = r.value as MediaConfig["imageProvider"];
    else if (r.key === "tts_engine") overrides.ttsEngine = r.value as MediaConfig["ttsEngine"];
    else if (r.key === "vision_provider") overrides.visionProvider = r.value as MediaSettingOverrides["visionProvider"];
  }
  return overrides;
}

/** Upsert one setting (unique on `key`). */
export async function saveMediaSetting(key: MediaSettingKey, value: string): Promise<void> {
  ...
  if (!res.ok) throw new Error(`Failed to save ${TABLE} (${res.status}): ${await res.text()}`);
}
