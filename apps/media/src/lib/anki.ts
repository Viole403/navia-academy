...
export function normalizeNote(note: AnkiNote, map: AnkiFieldMap): NormalizedNote | null {
...
export async function parseApkg(buffer: Buffer): Promise<AnkiDeck[]> {
  ...
  try {
    const decksRaw = db.prepare("SELECT id, name FROM decks").all() as { id: number; name: string }[];
    const deckById = new Map(decksRaw.map((d) => [d.id, d.name]));

    const rows = db
      .prepare(
        "SELECT id, mid, did, flds, tags FROM notes ORDER BY id",
      )
      .all() as { id: number; mid: number; did: number; flds: string; tags: string }[];
