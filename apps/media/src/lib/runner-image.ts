...
async function walkJson(dir: string): Promise<string[]> {
  const out: string[] = [];
  const entries = await readdir(dir, { withFileTypes: true }).catch(() => [] as { name: string; isDirectory(): boolean }[]);
 ...
 */
function conceptKey(item: Pick<VocabEntry, "translation" | "translationId">): string {
...
function lookupConceptRecord(
  ...
  const byId = item.translationId ? index.get(item.translationId.toLowerCase().trim()) : undefined;
  if (byId) return byId;
  return item.translation ? index.get(item.translation.toLowerCase().trim()) : undefined;
}

export async function generateImageBatch(opts: { limit?: number; dryRun?: boolean } = {}): Promise<ImageBatchResult> {
  ...
  const storageKeys = client ? await existingImageKeys(cfg, client) : new Set<string>();
  ...
  for (const item of items) {
    ...
    if (rec) {
      const ext = rec.ext ?? "";
      if ((await localImageExists(hash, ext)) || (ext && storageKeys.has(`images/${hash}.${ext}`))) {
        skipped++;
        newRecords.push(rec);
        continue;
      }
    ...
    try {
      ...
      if (!opts.dryRun) {
        ...
        if (client) {
          await uploadBuffer(cfg, client, `images/${hash}.${ext}`, toStore, mime);
        }
      }
      ...
      if (item.translationId) recordsByConcept.set(item.translationId.toLowerCase().trim(), record);
      if (item.translation) recordsByConcept.set(item.translation.toLowerCase().trim(), record);
      generated++;
    } catch (err) {
      errors++;
