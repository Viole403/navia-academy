...
import { dbConfigured, fetchPoolRows, patchPoolRowByProviderName, isRateLimitError, errMessage, cooldownMs, type PooledRow } from "./key-pool";
  ...
  if (dbConfigured()) {
    try {
      if (!poolCache || Date.now() - poolCache.fetchedAt > 60_000) {
        poolCache = { keys: await fetchPoolRows<VisionPoolRow>(POOL_TABLE), fetchedAt: Date.now() };
      }
      ...
      console.warn(`  ⚠ No enabled ${provider} vision keys in the pool — falling back to env`);
    } catch (err) {
      console.warn(`  ⚠ image_api_keys fetch failed (${errMessage(err)}) — falling back to env`);
    }
  }
  return [
    {
      provider: "gemini",
      name: "env",
      apiKey: process.env.GEMINI_API_KEY || process.env.MEDIA_IMAGE_API_KEY || "",
    },
  ];
}
...
async function geminiVision(key: ImageKeySpec, prompt: string, image: Buffer, mime: string): Promise<string> {
  const base = (key.apiBaseUrl || process.env.MEDIA_IMAGE_GEMINI_BASE_URL || "https://generativelanguage.googleapis.com/v1beta").replace(
    /\/+$/,
    "",
  );
  const model = key.model || process.env.GEMINI_MODEL || process.env.MEDIA_IMAGE_GEMINI_MODEL || "gemini-2.5-flash";
...
async function openaiVision(key: ImageKeySpec, prompt: string, image: Buffer, mime: string): Promise<string> {
  const base = (key.apiBaseUrl || process.env.MEDIA_IMAGE_API_BASE_URL || "https://api.openai.com/v1").replace(/\/+$/, "");
  const model = key.model || process.env.MEDIA_VISION_OPENAI_MODEL || "gpt-4o-mini";
...
/** Cloudflare Workers AI vision model (e.g. llama vision-instruct). */
async function cloudflareVision(key: ImageKeySpec, prompt: string, image: Buffer): Promise<string> {
  const account = key.cfAccountId || process.env.MEDIA_IMAGE_CF_ACCOUNT_ID;
  const model = key.model || process.env.MEDIA_VISION_CF_MODEL || "@cf/meta/llama-3.2-11b-vision-instruct-wasi";
  if (!account || !key.apiKey) throw new Error("Cloudflare vision: account id / token not set");
  ...
  if (!res.ok) throw new Error(`Cloudflare vision ${res.status}: ${await res.text()}`);
  const data = (await res.json()) as { result?: { response?: string } };
  return data.result?.response ?? "";
}

async function visionCall(key: ImageKeySpec, prompt: string, image: Buffer, mime: string): Promise<string> {
...
async function reportVisionStatus(key: ImageKeySpec, err?: unknown): Promise<void> {
  if (key.name === "env" || !dbConfigured()) return;
  const patch: Partial<VisionPoolRow> = { last_used_at: new Date().toISOString() };
  if (err) {
  ...
  await patchPoolRowByProviderName(POOL_TABLE, key.provider, key.name, patch as Record<string, unknown>).catch(() => undefined);
}
 ...
 */
export async function checkImageWithRotation(cfg: MediaConfig, concept: string, image: Buffer, mime: string): Promise<VisionVerdict> {
  void cfg;
  ...
  throw lastErr ?? new Error(`Vision check failed: no keys left for ${provider}`);
}
