...
export function loadMediaConfig(): MediaConfig {
  return {
    storageProvider: env(
      ...
      "s3"
    ) as MediaConfig["storageProvider"],
    ...
    imageProvider: env(
      ...
      "openai"
    ) as MediaConfig["imageProvider"],
    imageApiKey: env("MEDIA_IMAGE_API_KEY"),
    imageApiBaseUrl: env(
      "MEDIA_IMAGE_API_BASE_URL",
...
export async function resolveMediaConfig(): Promise<MediaConfig> {
  ...
  try {
    ...
    const imageProvider =
      env("MEDIA_IMAGE_PROVIDER") ||
      overrides.imageProvider ||
      cfg.imageProvider
    const ttsEngine =
      env("MEDIA_TTS_ENGINE") || overrides.ttsEngine || cfg.ttsEngine
    return {
      ...cfg,
