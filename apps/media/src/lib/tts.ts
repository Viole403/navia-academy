...
export type MediaLocale =
  "zh-CN" | "zh-TW" | "zh-HK" | "de-DE" | "ja-JP" | "en-US"
...
export async function synthesizeAudioWithKey(
  ...
  if (cfg.ttsEngine === "google")
    return googleTTS(text, locale, gender, key?.apiKey)
  if (cfg.ttsEngine === "azure")
    return azureTTS(text, locale, gender, key?.apiKey, key?.region)
  return edgeTTS(text, locale, gender)
}
...
async function googleTTS(
  ...
  if (!apiKey)
    throw new Error(
      "Google TTS: no API key (set GOOGLE_TTS_API_KEY or add a pooled 'google' key)"
    )
...
const AZURE_VOICES: Record<
  string,
  Record<MediaGender, { name: string; rate: string }>
> = {
  "zh-CN": {
...
async function azureTTS(
  ...
  if (!key)
    throw new Error(
      "Azure TTS: no API key (set AZURE_SPEECH_KEY or add a pooled 'azure' key)"
    )
