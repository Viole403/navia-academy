  ...
  if (dbConfigured()) {
    try {
      if (!poolCache || Date.now() - poolCache.fetchedAt > POOL_CACHE_TTL_MS) {
        poolCache = { keys: await fetchPoolRows<StoredKey>(POOL_TABLE), fetchedAt: Date.now() };
      }
      ...
      console.warn(`  ⚠ No enabled ${provider} keys in the pool (all in cooldown?) — falling back to env`);
    } catch (err) {
      console.warn(
 ...
 */
export async function reportKeyStatus(key: ImageKeySpec, err?: unknown): Promise<void> {
 ...
 */
export async function generateImageWithRotation(cfg: MediaConfig, prompt: string): Promise<Buffer> {
  ...
  throw lastErr ?? new Error(`Image generation failed: no keys left for ${provider}`);
}
...
export async function patchStoredKey(
  id: string,
  patch: Partial<Pick<StoredKey, "api_key" | "api_base_url" | "model" | "cf_account_id" | "enabled" | "cooldown_until" | "last_error">>,
): Promise<void> {
  if (!dbConfigured()) throw new Error("CONTENT_SUPABASE_URL + CONTENT_SUPABASE_SERVICE_ROLE_KEY not configured");
