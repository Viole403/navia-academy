...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(file instanceof File)) {
      return Response.json({ error: "multipart field 'file' (.apkg) is required" }, { status: 400 });
    }
    ...
    return Response.json({ error: err instanceof Error ? err.message : "anki import failed" }, { status: 500 });
  }
}
