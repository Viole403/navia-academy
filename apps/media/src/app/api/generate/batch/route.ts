...
export async function POST(request: NextRequest) {
  ...
  if (scope !== "audio" && scope !== "images" && scope !== "manifest" && scope !== "all") {
    return Response.json(
      { error: "INVALID_SCOPE", message: 'scope must be "audio" | "images" | "manifest" | "all"' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (!res.ok) {
      return Response.json(
        { error: "DISPATCH_FAILED", message: res.message ?? `GitHub returned ${res.status}` },
    ...
    return Response.json(
      { error: "GITHUB_UNREACHABLE", message: err instanceof Error ? err.message : "dispatch failed" },
      { status: 502 },
    );
  }
}
