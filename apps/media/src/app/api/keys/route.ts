...
export async function GET(request: NextRequest) {
  ...
  try {
    const keys = kind === "tts" ? await listTtsStoredKeys() : await listStoredKeys();
    return Response.json({ success: true, data: keys.map((k) => ({ ...k, api_key: maskKey(k.api_key) })) });
  } catch (err) {
  ...
  try {
    ...
    if (!body.provider || !body.name || !body.api_key) {
      return Response.json(
        { success: false, error: { code: "VALIDATION", message: "provider, name, and api_key are required" } },
        { status: 400 },
      );
    }
    const key =
    ...
    return Response.json({ success: true, data: { ...key, api_key: maskKey(key.api_key) } });
  } catch (err) {
    return Response.json({ success: false, error: { code: "KEYS_ERROR", message: err instanceof Error ? err.message : "failed" } }, { status: 500 });
  }
}
