...
interface Status {
  ...
  audioFiles: number;
  imageFiles: number;
  storage: { configured: boolean; provider: string; bucket: string; endpoint: string; publicUrl: string };
  ttsEngine: string;
...
export default function MediaStudioPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/anki/sources")
      .then((r) => r.json())
      .then((d) => {
        if (!cancelled && Array.isArray(d?.sources)) setAnkiSources(d.sources as AnkiSourceInfo[]);
      })
      .catch(() => {
  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold">Navia Media Studio</h1>
          <p className="text-sm" style={{ color: "var(--muted)" }}>
            Pusat data + pipeline audio &amp; gambar · zh (HSK/TOCFL) · de (Goethe) · en (TOEFL) · ja (JLPT)
          </p>
        </div>
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-3">
        <Stat label="Manifest entries" value={status ? String(status.manifestEntries) : "…"} />
        <Stat label="Audio files generated" value={status ? String(status.audioFiles) : "…"} />
        <Stat label="Image files generated" value={status ? String(status.imageFiles) : "…"} />
      </section>

      <section className="mt-6 rounded-xl p-5" style={{ background: "var(--panel)", border: "1px solid var(--border)" }}>
        <h2 className="font-semibold">Storage</h2>
        <pre className="mt-2 text-sm whitespace-pre-wrap" style={{ color: "var(--muted)" }}>
          {status
...
images   : ${status.imageProvider}`
      ...
      <section className="mt-6 rounded-xl p-5" style={{ background: "var(--panel)", border: "1px solid var(--border)" }}>
        ...
        <pre className="mt-2 overflow-x-auto text-sm" style={{ color: "var(--muted)" }}>
 ...
 pnpm data:import-anki -- file.apkg --dump out.json`}
      ...
      <section className="mt-6 rounded-xl p-5" style={{ background: "var(--panel)", border: "1px solid var(--border)" }}>
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--muted)" }}>
          Tambahkan deck di <code className="font-mono text-xs">apps/media/data/anki-sources.json</code> — isi URL/file
          + pemetaan kolom (<code className="font-mono text-xs">hanzi</code>, <code className="font-mono text-xs">pinyin</code>,{" "}
          <code className="font-mono text-xs">translation</code>, <code className="font-mono text-xs">zhuyin</code>) sesuai format tiap deck.
        </p>
        {ankiSources && ankiSources.length > 0 ? (
          ...
          <p className="mt-2 text-sm" style={{ color: "var(--muted)" }}>
            Belum ada sumber. Tambah di <code className="font-mono text-xs">apps/media/data/anki-sources.json</code>.
          </p>
        )}
      </section>
    </main>
...
function ActionCard({ title, desc, onRun, busy }: { title: string; desc: string; onRun: () => void; busy: boolean }) {
  return (
