...
import { parseApkg, normalizeNote, type AnkiFieldMap, type AnkiSource, type NormalizedNote } from "../src/lib/anki";
...
function parseFieldMap(arg?: string): AnkiFieldMap | null {
  ...
  for (const part of arg.split(",")) {
    ...
    const i = Number(v);
    if (!k || Number.isNaN(i)) throw new Error(`invalid field map part: ${part}`);
...
function reportSource(src: AnkiSource, decks: Awaited<ReturnType<typeof parseApkg>>, map: AnkiFieldMap, matched: NormalizedNote[]) {
  console.log(`\n[${src.id}] ${src.name} (${src.target ?? "?"})`);
  console.log(`  decks: ${decks.map((d) => `${d.name}(${d.notes.length})`).join(", ") || "—"}`);
  console.log(`  normalized notes: ${matched.length}`);
  const sample = matched.find((n) => n.hanzi);
  if (sample) console.log(`  sample: ${sample.hanzi} | ${sample.pinyin ?? "?"} | ${sample.translation ?? "?"} | ${sample.zhuyin ?? "?"}`);
}
...
async function importSource(src: AnkiSource) {
  ...
  const matched = notes.map((n) => normalizeNote(n, map)).filter((n): n is NonNullable<typeof n> => !!n);
...
async function main() {
  ...
  if (sourceIdIdx > -1) {
    ...
    if (!src) {
      ...
      console.error("Available:", sources.map((s) => s.id).join(", ") || "(none)");
      process.exit(1);
    }
  ...
  if (!file) {
    console.error("Usage: import-anki <file.apkg> [--fields hanzi=0,...] [--dump out.json] | --source <id> | --all");
    process.exit(1);
  }
  const buffer = await readFile(file);
  const decks = await parseApkg(buffer);
  const map = parseFieldMap(fieldMapArg) ?? { hanzi: 0 };
  const matched = decks.flatMap((d) => d.notes).map((n) => normalizeNote(n, map)).filter((n): n is NonNullable<typeof n> => !!n);
  console.log(`Decks: ${decks.map((d) => `${d.name}(${d.notes.length})`).join(", ")}`);
  console.log(`Normalized notes (field map ${JSON.stringify(map)}): ${matched.length}`);
  if (matched[0]) console.log(`sample: ${matched[0].hanzi} | ${matched[0].pinyin ?? "?"} | ${matched[0].translation ?? "?"} | ${matched[0].zhuyin ?? "?"}`);
  if (dumpPath) {
    const out = join(ROOT, dumpPath);
    await mkdir(join(out, ".."), { recursive: true });
    await writeFile(out, JSON.stringify(matched, null, 2), "utf-8");
