...
function itemFor(
  rec: ImageRecord,
  byTrans: Map<string, VocabEntry>,
  byId: Map<string, VocabEntry>
): VocabEntry | undefined {
  if (rec.translationId) {
    const id = byId.get(
      `${rec.language}::${rec.translationId.toLowerCase().trim()}`
    )
    if (id) return id
  }
  return rec.translation
    ? byTrans.get(`${rec.language}::${rec.translation.toLowerCase().trim()}`)
    : undefined
}
...
async function main() {
  ...
  for (const i of items) {
    ...
    if (i.translationId)
      byId.set(`${i.language}::${i.translationId.toLowerCase().trim()}`, i)
  }
  ...
  for (const rec of records) {
    ...
    row.translations.push(
      `${rec.translation}${rec.translationId ? ` (${rec.translationId})` : ""}`
    )
  ...
  if (validation) {
    for (const c of JSON.parse(
      await readFile(validation, "utf-8")
    ) as CheckResult[]) {
  ...
  const outRel = (row: Row) =>
    row.ext
      ? relative(
          dirname(outPath),
          join(OUTPUT_IMAGE_DIR, `${row.hash}.${row.ext}`)
        )
      : ""
  ...
  lines.push(
    `- Images: ${rows.length}/${byHash.size} on disk (records: ${records.length})`
  )
  lines.push(
    `- Languages: ${[...langCounts.entries()].map(([l, n]) => `${l}(${n})`).join(", ")}`
  )
  lines.push(
    `- Vision check: ${vision.size ? "yes" : "none — run `validate-images --json` and pass via --validation"}`
  )
  lines.push("")
  lines.push(
    "| # | Image | Word | Translation | Lang | Pos | Prompt | Vision |"
  )
  lines.push(
    "|---|-------|------|-------------|------|-----|--------|--------|"
  )
