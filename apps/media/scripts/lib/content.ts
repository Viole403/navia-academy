...
export async function buildLandingDemoBundle(): Promise<Bundle[]> {
  ...
  for (const lang of CONTENT_LANGS) {
    ...
    placementTotal += (
      JSON.parse(
        await readFile(join(JSON_DIR, lang, "placement.json"), "utf-8")
      ) as unknown[]
    ).length
  }
...
export async function writeLandingDemoStatic(bundles: Bundle[]): Promise<void> {
  ...
  const banner =
    ...
    "// Regenerate with `bun run --filter media publish-data` (or the CI publish step).\n"
...
export async function publishBundles(
  ...
  const publish = async (name: string, content: string) => {
    ...
    console.log(`✓ ${name} → ${key} (${(content.length / 1024).toFixed(1)} KB)`)
  }
