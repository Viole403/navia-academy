...
export async function fetchPublishedRows(
  ...
  if (!token) {
    fatal("CONTENT_EXPORT_TOKEN not set (see .env.example — must match apps/backend/.env).");
  }
...
function groupRowsByFile(rows: ExportRow[]): Map<string, ExportRow[]> {
  ...
  for (const row of rows) {
    const ref = row.ref && row.ref.trim() !== "" ? row.ref.replace(/\.json$/, "") : "contributor";
...
async function buildFilePlan(absPath: string, rows: ExportRow[]): Promise<FilePlan> {
  let existed = true;
  let current: unknown[];
  try {
...
async function writeWatermark(stats: SyncStats, lastSyncedAt: string | null): Promise<void> {
...
export async function runSync(opts: SyncOptions): Promise<SyncStats> {
  ...
  const scope = opts.lang || opts.domain ? ` (filters: ${[opts.lang, opts.domain].filter(Boolean).join("/")})` : "";
  console.log(`✓ fetched ${rows.length} published row(s)${scope}, total published: ${totalPublished}`);
  ...
  const stats: SyncStats = plans.reduce(
    ...
    { filesTouched: 0, added: 0, updated: 0, unchanged: 0, conflicts: 0, totalPublished },
  );
  ...
  for (const p of plans) {
    ...
    console.log(`  ${p.existed ? "~" : "+"} ${relPath(p.absPath)} (+${p.added} ~${p.updated})`);
  }
