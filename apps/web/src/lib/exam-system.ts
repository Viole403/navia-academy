...
export function generateMultipleChoiceQuestion(
  ...
  return {
    ...
    promptChinese: word.language === "zh" ? `"${script}" 是什么意思？` : undefined,
    options: shuffleArray([word.translation, ...distractors]),
    correctAnswer: word.translation,
    explanation: word.language === "zh" ? `${script} (${roman}) means "${word.translation}".` : `${script} means "${word.translation}".`,
...
export function generateFillBlankQuestion(
  ...
  return {
    ...
    promptChinese: word.language === "zh" ? `填空: "${blankedSentence}"` : undefined,
    correctAnswer: script,
    explanation: word.language === "zh" ? `The correct word is "${script}" (${roman}) which means "${word.translation}".` : `The correct word is "${script}" which means "${word.translation}".`,
    vocabularyId: word.id,
...
export function generateMatchingQuestion(
  ...
  return {
    ...
    prompt: words[0]?.language === "zh" ? "Match the Chinese words with their English meanings:" : "Match the words with their English meanings:",
    promptChinese: words[0]?.language === "zh" ? "匹配中文词语和英文意思:" : undefined,
    correctAnswer: pairs.map(p => `${p.chinese}:${p.english}`),
    explanation: pairs.map(p => `${p.chinese} (${p.pinyin}) = ${p.english}`).join("; "),
...
export function generatePronunciationQuestion(
  ...
  if (!roman) {
    ...
    throw new Error("Pronunciation questions only supported for languages with romanization (zh, ja)");
  }
  ...
  return {
    ...
    examLevel: level,
    hskLevel: (word.level ?? word.hsk ?? 1) as HskLevel,
    prompt: word.language === "zh" ? `What is the correct pinyin for "${script}"?` : `What is the correct reading for "${script}"?`,
    promptChinese: word.language === "zh" ? `"${script}" 的正确拼音是什么？` : undefined,
    options: shuffleArray([roman, ...options]),
    correctAnswer: roman,
    explanation: word.language === "zh" ? `The correct pinyin for ${script} is "${roman}".` : `The correct reading for ${script} is "${roman}".`,
    vocabularyId: word.id,
...
// Helper functions
function generateDistractors(word: VocabWord, difficulty: Difficulty): string[] {
...
function generatePinyinDistractors(word: VocabWord, difficulty: Difficulty): string[] {
  const distractors: string[] = [];
...
export function createExamSession(
  ...
  const questionsPerType = Math.ceil(settings.questionCount / settings.questionTypes.length);
  
  for (const questionType of settings.questionTypes) {
    for (let i = 0; i < questionsPerType && questions.length < settings.questionCount; i++) {
      const word = vocabulary[Math.floor(Math.random() * vocabulary.length)];
      const difficulty = settings.difficultyRange[
      ...
      switch (questionType) {
        case "multiple_choice":
          question = generateMultipleChoiceQuestion(word, examType, examLevel, difficulty);
          break;
        case "fill_blank":
          question = generateFillBlankQuestion(word, examType, examLevel, difficulty);
          break;
        case "pronunciation":
          question = generatePronunciationQuestion(word, examType, examLevel, difficulty);
          break;
        case "matching":
          // For matching, need multiple words
          const matchingWords = vocabulary.slice(0, 4); // Use first 4 words
          question = generateMatchingQuestion(matchingWords, examType, examLevel, difficulty);
          break;
        default:
  ...
  // Shuffle questions
  const shuffledQuestions = shuffleArray(questions).slice(0, settings.questionCount);
...
export function calculateScore(
  ...
  const byQuestionType: Record<QuestionType, { correct: number; total: number }> = {
  ...
  if (score >= 80) {
    const levels = ["1", "2", "3", "4", "5", "6", "7", "Novice 1", "Novice 2", "Level 1", "Level 2", "Level 3", "Level 4", "Level 5"];
...
function checkAnswer(question: ExamQuestion, userAnswer: string | string[] | undefined): boolean {
  ...
  switch (question.type) {
    ...
    case "matching":
      ...
      return userPairs.size === correctPairs.size && 
...
export function updateExamProgress(
  ...
  levelStats.averageScore = 
    (levelStats.averageScore * (levelStats.attempts - 1) + result.score) / levelStats.attempts;
  ...
  currentProgress.highestScore = Math.max(currentProgress.highestScore, result.score);
  currentProgress.totalAttempts++;
  currentProgress.averageScore = 
    (currentProgress.averageScore * (currentProgress.totalAttempts - 1) + result.score) / 
    currentProgress.totalAttempts;
  ...
  if (currentProgress.levelsCompleted.length > 0) {
    ...
    if (completedLevels.length > 0) {
      ...
      currentProgress.currentLevel = nextIndex < levels.length ? levels[nextIndex] : completedLevels[0];
    }
  }
