...
const SORTED_SYLLABLES = [...VALID_SYLLABLES].sort(
...
function syllableToZhuyin(raw: string): string {
  ...
  for (const jqx of ["j", "q", "x"]) {
    if (finalResolved.startsWith(jqx) && finalResolved[jqx.length] === "u") {
      finalResolved =
        finalResolved.slice(0, jqx.length) +
        "ü" +
        finalResolved.slice(jqx.length + 1)
      break
    }
  }
  ...
  const possibleFinals = Object.keys(FINAL_TO_ZHUYIN).sort(
    (a, b) => b.length - a.length
  )
  for (const fin of possibleFinals) {
    if (finalResolved.endsWith(fin)) {
      const potentialInit = finalResolved.slice(
        0,
        finalResolved.length - fin.length
      )
