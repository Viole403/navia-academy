 ...
 */
export function translationFor<T extends { translation?: string; translation_id?: string; translation_en?: string }>(
 ...
 */
export function locText<T>(obj: T | null | undefined, field: string | string[], locale: string): string {
  if (!obj) return "";
...
export function locArray<T>(obj: T | null | undefined, field: string, locale: string): string[] {
  if (!obj) return [];
  const v = obj as unknown as Record<string, unknown>;
  const pick = (val: unknown): string[] => (Array.isArray(val) ? (val as string[]).filter((s) => typeof s === "string") : []);
  if (locale === "id") {
