...
export function inExamFor(
  ...
  for (const c of chars)
    if (c.examMappings?.[exam]) keys.add(`character:${c.id}`)
  for (const g of grammar)
    if (g.examMappings?.[exam]) keys.add(`grammar:${g.id}`)
  return (c) => keys.has(`${c.kind}:${c.itemId}`)
}

export function todayLog(
  sessions: Record<string, StudySessionLog>
): StudySessionLog | null {
...
export function learnedCount(
  ...
  return Object.values(srs).filter(
    (c) => c.kind === kind && c.mastery >= minMastery
  ).length
}
