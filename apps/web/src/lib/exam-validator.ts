...
export class ExamValidator {
  ...
  static validateMappings(mappings: ExamMappings): ValidationResult {
    ...
    if (mappings.metadata) {
      if (
        ...
        !["official", "community", "auto-mapped", "manual"].includes(
          mappings.metadata.source
        )
      ) {
      ...
      if (
        mappings.metadata.confidence !== undefined &&
        (mappings.metadata.confidence < 0 || mappings.metadata.confidence > 1)
      ) {
  ...
  static validateWordStructure(word: VocabWord): ValidationResult {
    ...
    if (
      ...
      ![1, 2, 3, 4, 5].includes(word.frequency)
    ) {
