...
export const hydrateConversations = makeHydrator<ConversationScenario[]>(
  loadConversations,
  setData
)
...
export function useConversations(): ConversationScenario[] {
  const [snapshot, setSnapshot] = useState<ConversationScenario[]>(
    CONVERSATIONS.slice()
  )
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeConversations(() =>
      setSnapshot(CONVERSATIONS.slice())
    )
    return unsubscribe
  }, [])
