...
import { detectLocale, localeForExam, type VoiceGender, type VoiceLocale } from "@navia/utils";
...
function ensureAudioManifest(): Promise<void> {
  if (!manifestPromise) {
    manifestPromise = loadBundle<ManifestEntry[]>("audio/manifest").then(ingestAudioManifest);
  }
...
const DEFAULT_BASE =
  process.env.NEXT_PUBLIC_AUDIO_BASE_URL ?? "/audio";
const AUDIO_EXT = ".mp3";
...
const currentAudio = { el: null as HTMLAudioElement | null, key: "" as string | null, locale: null as VoiceLocale | null, gender: null as VoiceGender | null };
...
function savePersistentCache(): void {
  try {
    ...
    persistentCache.forEach((v, k) => { obj[k] = v; });
...
export function audioUrl(key: string, locale?: VoiceLocale, gender?: VoiceGender): string {
...
export function audioUrlWithGender(key: string, locale: VoiceLocale, gender: VoiceGender): string {
...
export function subscribeLoading(key: string, fn: (loading: boolean) => void): () => void {
...
function contentLocale(key: string, canonicalKey: string): VoiceLocale | undefined {
  return (localeByKey.get(key) ?? localeByKey.get(canonicalKey)) as VoiceLocale | undefined;
}
 ...
 *   Taiwanese voice.
 */
function effectiveLocale(key: string, canonicalKey: string, text: string): VoiceLocale {
...
export async function preloadAudio(key: string, locale?: VoiceLocale, gender?: VoiceGender): Promise<HTMLAudioElement> {
  ...
  const promise = (async () => {
    ...
    const cachedUrl = urlCache.get(urlCacheKey) ?? persistentCache.get(urlCacheKey);
    if (cachedUrl) {
      try {
...
export async function play(key: string, opts: AudioOpts = {}, locale?: VoiceLocale, gender?: VoiceGender) {
  if (!audioAvailable()) {
    opts.onError?.();
    return;
  }
  ...
  if (currentAudio.el && currentAudio.key === key && currentAudio.locale === locale && currentAudio.gender === genderKey) {
...
export function speak(text: string, opts: AudioOpts = {}) {
  ...
  const cached = (globalThis as unknown as { __cachedVoice?: SpeechSynthesisVoice }).__cachedVoice;
  ...
  if (cached) {
    ...
    if (picked) {
      utter.voice = picked;
      (globalThis as unknown as { __cachedVoice?: SpeechSynthesisVoice }).__cachedVoice = picked;
    }
  }
