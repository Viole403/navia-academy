...
export const EXAM_LANGS: Record<string, string> = {
  hsk: "zh",
  tocfl: "zh",
  goethe: "de",
  jlpt: "ja",
...
export async function loadManifest(): Promise<DataManifest> {
  if (
    manifestCache &&
    Date.now() - manifestCacheAt < MANIFEST_REVALIDATE * 1000
  )
...
export function loadLandingPreview(
  lang: LanguageCode = DEFAULT_LANGUAGE
): Promise<VocabWord[]> {
 ...
 */
export function examNameForLang(
  def: ExamDefinitionConfig,
  lang: string
): string {
...
export function loadPlacement<T = unknown>(
  lang: LanguageCode = DEFAULT_LANGUAGE
): Promise<T[]> {
