...
export function useExamCards(): ExamCardDef[] {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeExamCards(() => setSnapshot(EXAM_CARDS.slice()));
    return unsubscribe;
  }, []);
...
export function examCardByTypeLevel(examType: ExamType, level: string): ExamCardDef | undefined {
