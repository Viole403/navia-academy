...
export function newCard(itemId: string, kind: SrsItemKind, due = new Date()): SrsCard {
  return {
    itemId,
    kind,
    ease: DEFAULT_EASE,
...
export function gradeCard(card: SrsCard, grade: SrsGrade, now = new Date()): SrsCard {
  ...
  if (grade === 0) {
    ...
    if (grade === 1) {
      ...
      intervalDays =
        repetitions === 1 ? 2 : repetitions === 2 ? 5 : Math.round(intervalDays * ease * 1.3);
