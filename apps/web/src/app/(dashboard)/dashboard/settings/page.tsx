  ...
  function exportData() {
    const data = {
      ...
      progress: {
        ...
        savedWordIds: progress.savedWordIds,
        difficultItemIds: progress.difficultItemIds,
    ...
    <div className="animate-fade-up">
      ...
      {tab === "account" && (
        <div
          className="space-y-4"
          role="tabpanel"
          id="settings-tabs-panel-account"
          aria-labelledby="settings-tabs-tab-account"
          tabIndex={0}
        >
          <Card className="p-5">
          ...
          <Card className="p-5">
            ...
            {passwordMsg && (
              <p
                ...
                className={cn(
                  "mt-3 text-sm",
                  passwordMsg.ok ? "text-success" : "text-danger"
                )}
              >
                {passwordMsg.text}
              </p>
      ...
      {tab === "learning" && (
        <div
          className="space-y-4"
          role="tabpanel"
          id="settings-tabs-panel-learning"
          aria-labelledby="settings-tabs-tab-learning"
          tabIndex={0}
        >
          <Card className="p-5">
            <h2 className="font-display font-semibold">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              <Select
                ...
                value={String(s.dailyGoalMin)}
                onChange={(e) =>
                  s.set({ dailyGoalMin: Number(e.target.value) })
                }
              >
                {[15, 30, 45, 60, 90, 120].map((m) => (
                  <option key={m} value={m}>
                    {t("settings.minutes", { n: String(m) })}
                  </option>
                ))}
              </Select>
              <Select
                label={t("settings.newWordsPerDay")}
                value={String(s.newWordsPerDay)}
                onChange={(e) =>
                  s.set({ newWordsPerDay: Number(e.target.value) })
                }
              >
                {[4, 8, 12, 20].map((m) => (
                  <option key={m} value={m}>
              ...
              <Select
                ...
                onChange={(e) =>
                  s.set({ maxReviewsPerDay: Number(e.target.value) })
                }
              >
                {[20, 40, 60, 100, 200].map((m) => (
                  <option key={m} value={m}>
                    {m}
                  </option>
                ))}
          ...
          {(() => {
            const exams =
              LANGUAGES.find((l) => l.code === s.language)?.examTypes ?? []
            if (exams.length <= 1) return null
            return (
              <Card className="p-5">
                <h2 className="font-display font-semibold">
                  {t("settings.examProgram")}
                </h2>
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <Select
                    label={t("settings.targetExam")}
                    value={s.activeExamType}
                    onChange={(e) => {
                      const et = e.target.value as ExamType
                      s.set({ activeExamType: et })
                      if (s.language === "zh")
                        s.setDisplayMode({
                          script: et === "tocfl" ? "traditional" : "simplified",
                        })
                    }}
                  >
                    {exams.map((et) => (
                      <option key={et} value={et}>
                        {examConfig.abbreviations[et] ||
                          examConfig.displayNames[et] ||
                          et}
                      </option>
                    ))}
                  </Select>
          ...
          {s.language === "zh" && (
            <Card className="p-5">
              <h2 className="font-display font-semibold">
                {t("settings.chineseDisplay")}
              </h2>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <Select
                  label={t("settings.script")}
                  value={s.displayMode.script}
                  onChange={(e) =>
                    s.setDisplayMode({ script: e.target.value as "simplified" })
                  }
                >
                ...
                <Select
                  label={t("settings.hanziSize")}
                  value={s.hanziSize}
                  onChange={(e) => s.set({ hanziSize: e.target.value as "lg" })}
                >
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {orderedDisplayModes(
                    ...
                    <button
                      ...
                      className={cn(
                        "cursor-pointer rounded-[var(--radius)] border px-3.5 py-2.5 text-left text-sm transition-colors",
                        s.displayMode.mode === mode
                    ...
                    >
                      <span className="block font-medium">
                        {mode === "hanyu+trans" &&
                          t("settings.displayMode.hanyu+trans")}
                        {mode === "zhuyin+trans" &&
                          t("settings.displayMode.zhuyin+trans")}
      ...
      {tab === "appearance" && (
        <div
          className="space-y-4"
          role="tabpanel"
          id="settings-tabs-panel-appearance"
          aria-labelledby="settings-tabs-tab-appearance"
          tabIndex={0}
        >
          <Card className="p-5">
            <h2 className="font-display font-semibold">
          ...
          <Card className="p-5">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <Select
                ...
                value={s.fontSize}
                onChange={(e) => s.set({ fontSize: e.target.value as "md" })}
              >
              ...
              <Select
                ...
                value={s.density}
                onChange={(e) =>
                  s.set({ density: e.target.value as "comfortable" })
                }
              >
      ...
      {tab === "sound" && (
        <Card
          className="p-5"
          role="tabpanel"
          id="settings-tabs-panel-sound"
          aria-labelledby="settings-tabs-tab-sound"
          tabIndex={0}
        >
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <Select
              ...
              value={String(s.audioRate)}
              onChange={(e) => s.set({ audioRate: Number(e.target.value) })}
            >
          ...
          <div className="mt-2 divide-y divide-line">
            <Toggle
              ...
              onChange={(v) => s.set({ autoplayAudio: v })}
            />
            <Toggle
              label={t("settings.soundEffects")}
              description={t("settings.soundEffectsDesc")}
              checked={s.soundEffects}
              onChange={(v) => s.set({ soundEffects: v })}
            />
          </div>
          <p className="mt-4 text-xs text-ink-faint">
            Voice gender preference applies to all generated audio. Voices
            follow each lesson&apos;s locale: Chinese uses Mandarin (zh-CN) /
            Taiwanese Mandarin (zh-TW), German (de-DE), English (en-US),
            Japanese (ja-JP). Fallback: your browser&apos;s speech synthesis.
          </p>
        </Card>
      )}
      ...
      {tab === "notifications" && (
        <Card
          className="p-5"
          role="tabpanel"
          id="settings-tabs-panel-notifications"
          aria-labelledby="settings-tabs-tab-notifications"
          tabIndex={0}
        >
          <h2 className="font-display font-semibold">
          ...
          <div className="mt-2 divide-y divide-line">
            <Toggle
              ...
              onChange={(v) => s.set({ dailyReminder: v })}
            />
            <Toggle
              label={t("settings.streakAlerts")}
              description={t("settings.streakAlertsDesc")}
              checked={s.streakAlerts}
              onChange={(v) => s.set({ streakAlerts: v })}
            />
            <Toggle
              label={t("settings.weeklySummary")}
              description={t("settings.weeklySummaryDesc")}
              checked={s.weeklySummary}
              onChange={(v) => s.set({ weeklySummary: v })}
            />
          </div>
          {s.dailyReminder && (
          ...
          <p className="mt-4 text-xs text-ink-faint">
            Notifications are shown within the app. System push notifications
            require installing the app as a PWA and granting browser permission.
          </p>
        </Card>
      )}
      ...
      {tab === "privacy" && (
        <Card
          className="p-5"
          role="tabpanel"
          id="settings-tabs-panel-privacy"
          aria-labelledby="settings-tabs-tab-privacy"
          tabIndex={0}
        >
          <h2 className="font-display font-semibold">
            {t("settings.privacy")}
          ...
          <div className="mt-2 divide-y divide-line">
            <Toggle
              ...
              onChange={(v) => s.set({ publicProfile: v })}
            />
            <Toggle
              label={t("settings.visibleStats")}
              description={t("settings.visibleStatsDesc")}
              checked={s.showStats}
              onChange={(v) => s.set({ showStats: v })}
            />
          </div>
          <p className="mt-4 text-sm text-ink-soft">
            Your learning data is only used to personalize your itinerary. You
            can export or delete it in the Account tab. No analytics are
            activated without your consent.
          </p>
        </Card>
      )}
      ...
      {tab === "accessibility" && (
        <Card
          className="p-5"
          role="tabpanel"
          id="settings-tabs-panel-accessibility"
          aria-labelledby="settings-tabs-tab-accessibility"
          tabIndex={0}
        >
          <h2 className="font-display font-semibold">
          ...
          <ul className="mt-4 space-y-1 text-sm text-ink-soft">
            <li>
              · The entire app is keyboard navigable (Tab and arrows in
              lessons).
            </li>
            <li>· Audio always has visible text transcription.</li>
            <li>
              · Font and hanzi sizes can be adjusted in Appearance and Learning.
            </li>
          </ul>
        </Card>
      )}
      ...
      >
        <p className="text-sm text-ink-soft">{t("settings.deleteDesc")}</p>
