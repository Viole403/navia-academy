...
export default function ReadingPage() {
  ...
  const examReadings = useMemo(
    () =>
      READINGS.filter((r) =>
        Boolean(r.examMappings?.[settings.activeExamType])
      ),
  ...
  const filtered = useMemo(() => {
    ...
    return examReadings.filter((r) => {
      if (level !== "all") {
        const rLevel = String(
          r.examMappings?.[settings.activeExamType] ?? r.level ?? r.hsk
        )
        if (rLevel !== level) return false
      }
      if (type !== "all" && r.type !== type) return false
      if (q) {
        const text = (r.paragraphs ?? [])
          .map(
            (p) => `${p.hanzi ?? ""} ${p.pinyin ?? ""} ${p.translation ?? ""}`
          )
          .join(" ")
        const fields = [
          r.title,
          r.summary,
          text,
          locText(r, "title", locale),
          locText(r, "summary", locale),
        ]
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className="mb-5 flex flex-wrap gap-3">
        ...
        <Select
          aria-label={t("reading.levelFilter")}
          value={level}
        ...
        <Select
          aria-label={t("reading.typeFilter")}
          value={type}
            ...
            >
              ...
              <div className="mt-3 flex items-center gap-2 text-xs text-ink-faint">
                ...
                <span className="flex items-center gap-1">
                  ...
                  {t("reading.characters", { n: String(r.wordCount) })}
                </span>
              </div>
            </Link>
          ))
