...
function segment(text: string, vocabulary: VocabWord[]): { token: string; word: VocabWord | null }[] {
...
export default function ReadingDetailPage() {
  ...
  useFocusTrap({ active: selected !== null, containerRef: popupRef, onEscape: () => setSelected(null) });
  const mounted = useMounted();
  useReadings();
  const vocabulary = useVocabulary();

  const segments = useMemo(
    () => reading?.paragraphs.map((p) => segment(p.hanzi ?? p.text ?? "", vocabulary)) ?? [],
    [reading, vocabulary]
  );
  ...
  if (!reading) {
    return (
      <EmptyState
        ...
        action={<Link href="/dashboard/reading"><Button>{t("common.backTo", { page: t("nav.reading") })}</Button></Link>}
      />
    );
  }
  ...
  return (
    <div className="mx-auto max-w-2xl animate-fade-up">
      <Link href="/dashboard/reading" className="mb-4 inline-flex items-center gap-1.5 text-sm text-ink-faint hover:text-ink">
      ...
      <p className="mb-4 text-xs text-ink-faint">Tap any underlined word to view its card and save it.</p>

      <div className="space-y-5">
        {reading.paragraphs.map((p, i) => (
          <Card key={i} className="p-5">
            <div className="flex items-start justify-between gap-3">
              <p className={cn("hanzi leading-relaxed", settings.hanziSize === "xl" ? "text-3xl" : settings.hanziSize === "lg" ? "text-2xl" : "text-xl")} lang="zh-CN">
              ...
              <button
                onClick={() => play(p.audio ?? p.hanzi ?? p.text ?? "", { rate: settings.audioRate, onLoadingChange: setAudioLoading, onError: () => {} }, "zh-CN", settings.voiceGender)}
                disabled={audioLoading}
                className="mt-1 shrink-0 rounded p-1.5 text-ink-faint hover:text-accent cursor-pointer disabled:opacity-50"
                aria-label={t('audio.listenParagraph')}
              >
                <Volume2 className="h-4 w-4" />
              </button>
            </div>
            {<PinyinText pinyin={p.pinyin ?? p.romanization ?? ""} zhuyin={p.zhuyin} className="mt-2 block text-sm" />}
            {showsTranslation(settings.displayMode.mode) && <p className="mt-1 text-sm text-ink-faint">{translationFor(p, locale)}</p>}
          </Card>
        ))}
      </div>
      ...
      {/* Word popup */}
      {selected && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center" onClick={(e) => e.target === e.currentTarget && setSelected(null)}>
          <div
            ref={popupRef}
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="relative">
              <button
                ...
                className="absolute -top-2 right-2 z-10 rounded-full border border-line bg-raised p-1.5 text-ink-faint hover:text-ink cursor-pointer"
              >
                <X className="h-4 w-4" />
              </button>
