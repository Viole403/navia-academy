    ...
    <div className="animate-fade-up">
        ...
        action={
          <Link href="/dashboard/settings">
            <Button variant="outline" size="sm">
              <Settings className="h-4 w-4" /> {t("profile.editSettings")}
            </Button>
          </Link>
        }
      />
      ...
      <Card className="p-6">
        <div className="flex flex-wrap items-center gap-5">
          <span
            className="flex h-20 w-20 items-center justify-center rounded-full bg-accent text-2xl font-bold text-accent-ink"
            aria-hidden
          >
            {initials}
          </span>
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              {progress.placement && (
                <Badge>
                  {GENERAL_LEVEL_LABELS[progress.placement.generalLevel]}
                </Badge>
              )}
              <Badge tone="gold">
                {t("profile.level", {
                  level: String(progress.levelFromXp()),
        ...
        {progress.startedAt && (
          <p className="mt-4 flex items-center gap-1.5 text-xs text-ink-faint">
            ...
            {t("profile.studyingSince", {
              date: new Date(progress.startedAt).toLocaleDateString("en", {
                day: "numeric",
                month: "long",
                year: "numeric",
              }),
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          sub={t("profile.bestStreak", { n: String(progress.bestStreak) })}
          icon={<Flame className="h-4 w-4" />}
        />
        <StatCard
          label={t("dashboard.words")}
          value={learnedCount(srsFor(progress, language), "word")}
        />
        <StatCard
        ...
        <Card className="mt-4 p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {unlockedAchievements.slice(0, 12).map((a) => (
              <span
                key={a.id}
                className="bg-surface flex h-12 w-12 items-center justify-center rounded-[var(--radius)] border border-line text-accent"
                title={a.title}
              >
      ...
      {progress.placement && (
        <Card className="mt-4 p-5">
          <h2 className="font-display font-semibold">
            {t("profile.latestAssessment")}
          </h2>
          <p className="mt-2 text-sm text-ink-soft">
            {t("profile.assessmentSummary", {
              date: new Date(progress.placement.takenAt).toLocaleDateString(
                "en"
              ),
              correct: String(progress.placement.correctCount),
              answered: String(progress.placement.answeredCount),
              level:
                progress.placement.confidence === "high"
                  ? t("profile.confidence.high")
                  : progress.placement.confidence === "medium"
                    ? t("profile.confidence.medium")
                    : t("profile.confidence.low"),
            })}
          </p>
          <Link href="/dashboard/placement-test" className="mt-3 inline-block">
            <Button variant="outline" size="sm">
              {t("profile.retakePlacement")}
            </Button>
          </Link>
        </Card>
      )}
