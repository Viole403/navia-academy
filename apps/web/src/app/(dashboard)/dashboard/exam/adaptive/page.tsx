...
import { useCatExam, catRecommendedLevel, type CatResume, type CatAnswerLog, type CatItemFormat } from "@/hooks/use-cat-exam";
...
function toWire(a: CatAnswerLog): WireAnswer {
  return { item_id: a.wordId, item_elo: a.elo, correct: a.correct, format: a.format };
}

/** Wire → engine log (`item_id/item_elo` → `wordId/elo`). */
function toLog(a: WireAnswer): CatAnswerLog {
  return { wordId: a.item_id, elo: a.item_elo, correct: a.correct, format: a.format as CatItemFormat };
}
...
export default function AdaptiveExamPage() {
  ...
  const { current, theta, log, done, result, start, answer } = useCatExam(undefined, resumeObj);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const prog = (await api<Array<{ elo_estimate: number }>>("/cat/progress")) as Array<{ elo_estimate: number }>;
        if (prog.length > 0 && !window.localStorage.getItem("navia-cat-last-elo")) {
          window.localStorage.setItem("navia-cat-last-elo", String(Math.round(prog[0].elo_estimate)));
        }
      } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const s = (await api<CatSessionDTO>(`/cat/session/${raw}`)) as CatSessionDTO;
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      if (
        !timedOutRef.current &&
        (qElapsedRef.current >= PER_QUESTION_SECONDS || elapsedRef.current >= maxSeconds)
      ) {
  ...
  useEffect(() => {
    ...
    void api(`/cat/session/${sid}`, { method: "PATCH", body: JSON.stringify(payload) }).catch(() => {});
  }, [log, done, theta]);
  ...
  function beginSession(resumeData: CatResume | null) {
    ...
    api<{ id: number }>("/cat/session", {
      method: "POST",
      body: JSON.stringify({ exam_type: examType, start_theta: theta, time_limit_sec: maxSeconds }),
    })
      .then((s: { id: number }) => {
        sessionIdRef.current = s.id;
  ...
  useEffect(() => {
    const onHide = () => {
      ...
      void api(`/cat/session/${sid}`, {
        method: "PATCH",
        body: JSON.stringify({ answers: log.map(toWire), elapsed_sec: elapsedRef.current, theta: Math.round(theta) }),
      }).catch(() => {});
    };
  ...
  function restart() {
    ...
    api<{ id: number }>("/cat/session", {
      method: "POST",
      body: JSON.stringify({ exam_type: examType, start_theta: 550, time_limit_sec: maxSeconds }),
    })
      .then((s: { id: number }) => {
        sessionIdRef.current = s.id;
        window.localStorage.setItem(SESSION_KEY, String(s.id));
  ...
  if (ready && pendingResume && !done) {
    return (
      <main className="mx-auto flex min-h-screen max-w-xl flex-col justify-center px-4 py-10 text-center">
        ...
        <h1 className="font-display mt-4 text-2xl font-bold">{t("cat.resumeTitle")}</h1>
        <p className="mt-2 text-sm text-ink-faint">
          {t("cat.resumeDesc", { n: String(pendingResume.answers.length) })}
        </p>
        <div className="mt-6 flex gap-3">
          <Button className="flex-1" onClick={() => beginSession(pendingResume)}>
            {t("cat.resumeContinue")}
          </Button>
          <Button variant="outline" className="flex-1" onClick={() => beginSession(null)}>
            {t("cat.resumeNew")}
          </Button>
        </div>
      </main>
    ...
    return (
      <main className="mx-auto max-w-xl px-4 py-10 animate-fade-up">
        <div className="text-center">
          ...
          <h1 className="font-display mt-4 text-2xl font-bold">{t("cat.yourResult")}</h1>
          <p className="mt-1 text-sm text-ink-faint">
            {t("cat.summary", { answered: String(result.answered), correct: String(result.correct) })}
          </p>
        </div>

        <div className="mt-6 grid grid-cols-2 gap-3">
          <div className="rounded-[var(--radius)] border border-line p-4 text-center">
            <p className="text-xs uppercase tracking-wide text-ink-faint">{t("cat.cefrBand")}</p>
            <p className="font-display mt-1 text-3xl font-bold text-accent">{result.cefrBand}</p>
          </div>
          <div className="rounded-[var(--radius)] border border-line p-4 text-center">
            <p className="text-xs uppercase tracking-wide text-ink-faint">{t("cat.eloScore")}</p>
            <p className="font-display mt-1 text-3xl font-bold">{Math.round(result.eloEstimate)}</p>
            <p className="mt-1 text-xs text-ink-faint">±{Math.round(result.eloSd)}</p>
          </div>
        </div>

        <div className="mt-3 rounded-[var(--radius)] border border-line p-4 text-center">
          <p className="text-xs uppercase tracking-wide text-ink-faint">{t("cat.recommendedLevel")}</p>
        ...
        {result.weakBands.length > 0 && (
          <div className="mt-3 rounded-[var(--radius)] border border-line p-4">
            <p className="text-xs uppercase tracking-wide text-ink-faint">{t("cat.weakAreas")}</p>
            <div className="mt-2 flex flex-wrap gap-2">
              {result.weakBands.map((b) => (
                <Badge key={b} tone="warn">{b}</Badge>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="mt-6 flex gap-3">
          <Button className="flex-1" onClick={restart}>
            <RotateCcw className="h-4 w-4 mr-1" /> {t("cat.retake")}
          </Button>
          <Button variant="outline" className="flex-1" onClick={() => router.push("/dashboard/exam")}>
            {t("common.done")}
          </Button>
        </div>
      </main>
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-xl flex-col px-4 py-10">
      <div className="mb-8">
        <div className="flex items-center justify-between text-xs text-ink-faint">
          ...
          <span className="flex items-center gap-3">
            <span>{formatTime(elapsed)} / {formatTime(maxSeconds)}</span>
            <span>Θ {Math.round(theta)}</span>
          </span>
        </div>
        <ProgressBar className="mt-2" value={log.length} max={Math.max(log.length, 1)} label={t("cat.progress")} />
        {tabWarnings >= 1 && (
          <p className="mt-2 text-xs text-danger">{t("cat.tabWarning", { n: String(tabWarnings) })}</p>
        )}
      ...
      {current && (
        <div className="animate-brush-in" key={`${current.word.id}-${current.format}`}>
          ...
          {current.prompt && (
            <h1 className="font-display mt-2 text-xl font-semibold">{current.prompt}</h1>
          )}

          {current.format === "listening" && (
            <button
              onClick={() =>
                play(current.audioText ?? current.correctAnswer, {
                  rate: settings.audioRate,
                  onLoadingChange: setAudioLoading,
                  onError: () => {},
                }, ttsLocale, settings.voiceGender)
              }
            ...
            >
              ...
              {audioLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : t("cat.playAudio")}
            </button>
          )}
