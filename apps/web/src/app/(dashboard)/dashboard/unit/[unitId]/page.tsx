...
import { useCurriculum, lessonsOfUnit, levelById, unitById } from "@/lib/curriculum";
...
export default function UnitPage() {
  ...
  if (!unit) {
    return (
      <EmptyState
        ...
        action={<Link href="/dashboard/program"><Button>{t("unit.backToProgram")}</Button></Link>}
      />
    );
  }
  ...
  return (
    <div className="animate-fade-up">
      <Link href="/dashboard/program" className="mb-4 inline-flex items-center gap-1.5 text-sm text-ink-faint hover:text-ink">
        ...
        action={
          next && (
            <Link href={`/dashboard/lesson/${next.id}`}>
              <Button><Play className="h-4 w-4" /> {lessons[next.id] ? t("lesson.continue") : t("unit.start")}</Button>
            </Link>
          )
        }
      />
      <div className="mb-4 flex flex-wrap gap-2">
        {unit.skills?.map((s) => <Badge key={s}>{SKILL_LABELS[s]}</Badge>)}
      </div>
          ...
          return (
            <Link key={lesson.id} href={`/dashboard/lesson/${lesson.id}`}>
              <Card className="mb-3 flex items-center gap-4 p-4 transition-colors hover:border-accent">
                {completed ? (
                  <CheckCircle2 className="h-6 w-6 shrink-0 text-success" aria-label={t("unit.completed")} />
                ) : (
                  <Circle className="h-6 w-6 shrink-0 text-ink-faint" aria-label={t("unit.pending")} />
                )}
                ...
                <div className="flex shrink-0 items-center gap-3 text-xs text-ink-faint">
                  {inProgress && <Badge tone="warn">{t("unit.inProgress")}</Badge>}
                  <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {t("unit.minutes", { n: String(lesson.durationMin) })}</span>
