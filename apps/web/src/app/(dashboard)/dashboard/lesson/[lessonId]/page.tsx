...
export default function LessonPage() {
      ...
      <EmptyState
        ...
        action={
          <Link href="/dashboard/program">
            <Button>{t("lesson.viewProgram")}</Button>
          </Link>
        }
      />
    )
  ...
  const exerciseDone =
    step.type !== "exercise" ||
    step.exercise === undefined ||
    step.exercise.id in exerciseResults
  const difficult = difficultFor(progress, settings.language).includes(
    lesson.id
  )
    ...
    if (!finished) {
      ...
      for (const s of lesson!.steps) {
        ...
        for (const cid of s.characterIds ?? [])
          progress.ensureCard(cid, "character")
        for (const gid of s.grammarIds ?? [])
          progress.ensureCard(gid, "grammar")
      }
      if (!alreadyDone) {
        progress.completeLesson(lesson!.id, lesson!.xp)
        progress.logStudy(
          lesson!.durationMin,
          lesson!.skills[0] ?? "vocabulary",
          0
        )
      }
  ...
  if (finished) {
    ...
    return (
      <div className="animate-fade-up mx-auto max-w-xl py-10 text-center">
        ...
        <p className="mt-2 text-ink-soft">
          ...
          {total > 0 &&
            ` · ${t("lesson.correctExercises", { correct: String(correct), total: String(total) })}`}
        </p>
        <p className="mt-1 text-sm text-ink-faint">
          {t("lesson.addedToReview")}
        </p>
        <div className="mt-8 flex justify-center gap-3">
          <Link href="/dashboard/review">
            <Button variant="outline">{t("lesson.reviewNow")}</Button>
          </Link>
          <Link href="/dashboard/learn">
            <Button>
              {t("dashboard.nextLesson")} <ArrowRight className="h-4 w-4" />
            </Button>
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className={cn(
              "cursor-pointer rounded-lg p-2 hover:bg-hover",
              difficult ? "text-danger" : "text-ink-faint"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "cursor-pointer rounded-lg p-2 hover:bg-hover",
              showNotes ? "text-accent" : "text-ink-faint"
            )}
          >
      ...
      {showNotes && (
        <Card className="mb-5 p-4">
          <Textarea
            ...
            defaultValue={
              notesFor(progress, settings.language)[lesson.id] ?? ""
            }
      ...
      {/* Step content */}
      <Card className="animate-brush-in p-6" key={step.id}>
        ...
        {step.body?.map((p, i) => (
          <p
            key={i}
            className="mt-3 text-sm leading-relaxed text-ink-soft first:mt-0"
          >
            {p}
          </p>
        ))}
          ...
          <div className="mt-4 space-y-3">
            {step.dialogue.map((line, i) => (
              <div key={i} className="flex gap-3">
                <span
                  className="hanzi mt-1 h-8 w-8 shrink-0 rounded-full bg-sunken text-center text-sm leading-8"
                  lang={locale}
                >
                ...
                <div className="min-w-0 flex-1 rounded-[var(--radius)] border border-line bg-sunken/40 px-4 py-2.5">
                  <div className="flex items-start justify-between gap-2">
                    <p className="hanzi text-lg" lang={locale}>
                      {line.hanzi ?? line.text ?? ""}
                    </p>
                    <button
                      onClick={() =>
                        play(
                          line.audio ?? line.hanzi ?? line.text ?? "",
                          {
                  ...
                  {
                    <PinyinText
                      pinyin={line.pinyin ?? line.romanization ?? ""}
                      zhuyin={line.zhuyin}
                      className="text-xs"
                    />
                  }
        ...
        {step.type === "reading" && step.paragraphs && (
          <div className="mt-4 space-y-4">
            {step.paragraphs.map((para, i) => (
              <div
                key={i}
                className="rounded-[var(--radius)] border border-line bg-sunken/40 px-4 py-3.5"
              >
                <div className="flex items-start justify-between gap-2">
                  <p className="hanzi text-lg" lang={locale}>
                    {para.hanzi ?? ""}
            ...
            {step.grammarIds.map((id) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="hanzi mt-1 text-sm text-accent" lang={locale}>
                    {g.pattern}
                  </p>
                  <p className="mt-2 text-sm text-ink-soft">
                    {locText(g, "simpleExplanation", uiLocale)}
                  </p>
                  {g.examples[0] && (
                    <p className="hanzi mt-2 text-sm" lang={locale}>
                      {g.examples[0].hanzi}{" "}
                      <span className="font-sans text-xs text-ink-faint">
                        — {translationFor(g.examples[0], uiLocale)}
                      </span>
                    </p>
                  )}
                </div>
              ...
              return (
                <div key={id} className="text-center">
                  <p className="mb-2 text-sm text-ink-soft">
                    <span className="hanzi text-lg" lang={locale}>
                      {c.char}
                    </span>{" "}
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <p className="hidden text-xs text-ink-faint sm:block">
          {t("lesson.navigationHint")}
        </p>
        <Button onClick={goNext} disabled={!exerciseDone}>
          {isLast ? t("lesson.finish") : t("assessment.next")}{" "}
