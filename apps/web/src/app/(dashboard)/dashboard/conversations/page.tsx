...
export default function ConversationsPage() {
  ...
  const {
    ...
    displayNames: EXAM_DISPLAY_NAMES,
    abbreviations: EXAM_ABBREVIATIONS,
  } = useExamConfig()
  ...
  const filtered = useMemo(() => {
    ...
    return examConversations.filter((c) => {
      if (level !== "all") {
        const cLevel = String(
          c.examMappings?.[settings.activeExamType] ?? c.level ?? c.hsk
        )
        if (cLevel !== level) return false
      }
      if (!q) return true
      const turnText = (c.turns ?? [])
        .map((tr) => `${tr.text} ${tr.translation} ${tr.romanization ?? ""}`)
        .join(" ")
      const fields = [
        c.title,
        c.context,
        turnText,
        locText(c, "title", locale),
        locText(c, "context", locale),
      ]
  ...
  return (
    <div className="animate-fade-up">
      <SectionHeader
        ...
        subtitle={t("conversation.scenarios", {
          n: String(examConversations.length),
      ...
      <div className="mb-5 flex flex-wrap gap-3">
        ...
        <Select
          aria-label={t("conversation.levelFilter")}
          value={level}
          onChange={(e) => setLevel(e.target.value)}
          className="w-44"
        >
          <option value="all">
            {t("conversation.allLevels", {
              exam:
                EXAM_ABBREVIATIONS[settings.activeExamType] ||
                EXAM_DISPLAY_NAMES[settings.activeExamType],
            })}
          </option>
          {examDef.levels.map((l: string) => (
            <option key={l} value={l}>
              {t("conversation.levelOption", {
                exam:
                  EXAM_ABBREVIATIONS[settings.activeExamType] ||
                  EXAM_DISPLAY_NAMES[settings.activeExamType],
                level: l,
          ...
          {visible.map((c) => (
            ...
            >
              ...
              <div className="mt-3 flex gap-2 text-xs">
                <Badge>
                  {c.formality === "formal"
                    ? t("conversation.formal")
                    : c.formality === "informal"
                      ? t("conversation.informal")
                      : t("conversation.neutral")}
                </Badge>
                <Badge>
                  <MessageCircle className="h-3 w-3" />{" "}
                  {t("conversation.interventions", {
                    n: String(
                      (c.turns ?? []).filter((tr) => tr.speaker === "user")
                        .length
                    ),
                  })}
                </Badge>
