...
export default function ConversationPlayerPage() {
  ...
  return (
    <div className="animate-fade-up mx-auto max-w-xl">
      ...
      <SectionHeader
        title={locText(conv, "title", locale)}
        subtitle={locText(conv, "context", locale)}
      />
          ...
          >
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <p className="hanzi text-lg" lang="zh-CN">
                  {item.hanzi}
                </p>
                <button
                  onClick={() =>
              ...
              {item.feedback && (
                <p
                  className={cn(
                    "mt-2 border-t border-line pt-2 text-xs",
                    item.natural ? "text-success" : "text-warn"
                  )}
                >
                  {item.feedback}
                </p>
      ...
      {currentTurn?.choices && (
        <Card className="mt-5 p-5">
          <p className="text-sm font-medium">
            Your turn: choose the most natural response
          </p>
            ...
            {choiceOrder.map((choiceIdx) => {
              ...
              return (
                <button
                  ...
                  className="w-full cursor-pointer rounded-xl border border-line bg-raised px-4 py-3 text-left hover:border-accent hover:bg-hover"
                >
                  <span className="hanzi text-base" lang="zh-CN">
                    {c.hanzi ?? ""}
                  </span>
                  <PinyinText
                    pinyin={c.pinyin ?? ""}
      ...
      {finished && (
        <div className="mt-6 text-center">
          ...
          <div className="mt-4 flex justify-center gap-3">
            ...
            <Link href="/dashboard/conversations">
              <Button>{t("conversation.newScenario")}</Button>
            </Link>
          </div>
        </div>
      )}
