  ...
  const notices = useMemo<Notice[]>(() => {
    ...
    if (due.length > 0) {
      out.push({
        id: "due",
        icon: Bell,
        title: t("notifications.pendingReviews", { n: String(due.length) }),
        body: t("notifications.dueBody"),
      })
    }
    const overdue = progress.tasks.filter(
    ...
    if (overdue.length > 0) {
      out.push({
        id: "overdue",
        icon: ClipboardList,
        title: t("notifications.overdueTasks", { n: String(overdue.length) }),
    ...
    if (
      ...
      out.push({
        id: "streak",
        icon: Flame,
        title: t("notifications.streakAtStake", { n: String(progress.streak) }),
    ...
    for (const [id, at] of Object.entries(progress.achievements)
      ...
      if (a)
        out.push({
          id: `ach-${id}`,
          icon: Award,
          title: t("notifications.achievementUnlocked", { name: a.title }),
              ...
              <div>
                ...
                {n.at && (
                  <p className="mt-0.5 text-xs text-ink-faint">
                    {new Date(n.at).toLocaleDateString("en", {
                      day: "numeric",
                      month: "long",
                    })}
                  </p>
