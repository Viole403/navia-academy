...
export default function BrowseClient({
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      loadBundle<unknown[]>(dataset.bundle)
        .then((data) => {
          if (seq.current === id)
            setLoaded((prev) => ({ ...prev, [dataset.key]: data }))
        })
        .catch(() => {
          if (seq.current === id)
            setLoaded((prev) => ({ ...prev, [dataset.key]: [] }))
        })
        .finally(() => {
  ...
  const cols = useMemo(() => {
    ...
    return [
      ...
      {
        label: t("contrib.browse.colPronunciation"),
        value: reading,
        mono: false,
      },
      translationCol,
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <Badge tone="neutral">
          {t("contrib.browse.shown", { n: String(filtered.length) })}
        </Badge>
      </div>
                    ...
                    >
                      {cols.map((c) => (
                        <td
                          ...
                          className={
                            c.mono
                              ? "px-4 py-2 font-mono text-xs text-ink-faint"
                              : "px-4 py-2 text-ink"
                          }
                        >
                          {c.value(o)}
