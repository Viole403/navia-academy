...
const DOMAINS = ["vocabulary", "grammar", "readings", "conversations", "characters"] as const;
  ...
  const submitForReview = async (row: ContentRow) => {
    try {
      await api(`/api/v1/content/${row.lang}/${row.domain}/${row.id}`, {
        method: "PUT",
        body: JSON.stringify({ status: "review", expected_updated_at: row.updated_at }),
      });
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-ink-soft">
            {isReviewer ? "Reviewer: you can publish or reject submitted items." : "Create and submit content — a reviewer approves it for the CDN."}
          </p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <label className="flex items-center gap-2 text-sm text-ink-soft">
          <input type="checkbox" checked={showOnlyMine} onChange={(e) => setShowOnlyMine(e.target.checked)} className="accent-accent" />
          Only mine
        </label>
        ...
        <Button variant="outline" size="sm" onClick={fetchRows}><RefreshCw className="h-4 w-4" /></Button>
      </div>
      ...
      {error && (
        <div className="mb-4 rounded-lg border border-danger/30 bg-danger/5 px-4 py-3 text-sm text-danger">{error}</div>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-line p-10 text-center text-ink-soft">
          No items here yet. Click <span className="font-medium text-accent">New item</span> to contribute.
        </div>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-line">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-line">
              {filtered.map((r) => (
                <tr key={`${r.lang}/${r.domain}/${r.id}`} className="hover:bg-sunken/50">
                  <td className="px-4 py-3 font-medium text-ink">{payloadSummary(r.payload)}</td>
                  <td className="px-4 py-3 text-ink-faint text-xs">{r.id}</td>
                  <td className="px-4 py-3">
                    <Badge tone={STATUS_TONE[r.status] ?? "neutral"}>{r.status}</Badge>
                    {r.status === "rejected" && r.review_note && (
                      <p className="mt-1 text-xs text-amber-700">{r.review_note}</p>
                    )}
                  </td>
                  <td className="px-4 py-3 text-ink-faint text-xs">{new Date(r.updated_at).toLocaleString()}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      ...
                      {r.status !== "review" && (
                        <button
                          ...
                          className="rounded p-1.5 text-ink-faint hover:bg-hover hover:text-accent cursor-pointer"
      ...
      {creating && (
        <ContentEditor
          lang={lang}
          domain={domain}
          onClose={() => { setCreating(false); fetchRows(); }}
        />
      )}
      {editing && (
        <ContentEditor
          ...
          row={editing}
          onClose={() => { setEditing(null); fetchRows(); }}
        />
      )}

      {toast && (
        <div className="fixed bottom-4 right-4 rounded-lg bg-ink px-4 py-2 text-sm text-accent-ink shadow-lg">{toast}</div>
      )}
    </div>
  );
}
...
function ContentEditor({
  ...
  const [payloadText, setPayloadText] = useState(row ? JSON.stringify(row.payload, null, 2) : "");
  ...
  const save = async () => {
    ...
    try {
      await api(isEdit ? `/api/v1/content/${row!.lang}/${row!.domain}/${row!.id}` : `/api/v1/content`, {
        method: isEdit ? "PUT" : "POST",
        body: JSON.stringify(isEdit
          ? { payload, expected_updated_at: row!.updated_at, ref: ref || undefined }
  ...
  return (
    <Modal open onClose={onClose} title={isEdit ? "Edit item" : `New ${domain} item (${lang})`}>
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-ink-soft">ID (unique per lang/domain)</label>
          <Input value={id} onChange={(e) => setId(e.target.value)} disabled={isEdit} placeholder="e.g. new-word" />
        </div>

        <div>
          <label className="block text-sm font-medium text-ink-soft">Suggested level (reviewer will confirm)</label>
          ...
          <p className="mt-1 text-xs text-ink-faint">Optional proposal — the reviewer assigns the final target file on approval.</p>
        </div>

        {!payloadText &&
          fields.map((f) => (
            <div key={f.key}>
              <label className="block text-sm font-medium text-ink-soft">{f.label}</label>
              {f.list ? (
                <Textarea
                  value={values[f.key] ?? ""}
        ...
        <div>
          <label className="block text-sm font-medium text-ink-soft">Payload JSON (advanced)</label>
          <Textarea
            value={payloadText}
          ...
          />
          <p className="mt-1 text-xs text-ink-faint">Fill the guided fields above, or paste the full item payload as JSON.</p>
        </div>

        {err && <div className="rounded-lg border border-danger/30 bg-danger/5 px-3 py-2 text-sm text-danger">{err}</div>}

        <div className="flex justify-end gap-2 pt-2">
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button loading={saving} onClick={save}>{isEdit ? "Save" : "Create draft"}</Button>
        </div>
      </div>
    </Modal>
  );
