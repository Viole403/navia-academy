...
import { EmptyState, PinyinText, Pagination, SectionHeader, Select } from "@/components/ui";
...
export default function CharactersPage() {
  ...
  const examChars = useMemo(
    () => CHARACTERS.filter((c) => Boolean(c.examMappings?.[settings.activeExamType])),
    [settings.activeExamType, CHARACTERS],
  );
  const radicals = useMemo(() => [...new Set(examChars.map((c) => c.radical))].sort(), [examChars]);
  ...
  const filtered = useMemo(() => {
    ...
    return examChars.filter((c) => {
      ...
      if (q) {
        ...
        const pinyin = stripPinyinTones(c.pinyin).toLowerCase().includes(qToneFree);
  ...
  return (
    <div className="animate-fade-up">
      <SectionHeader
        ...
        subtitle={t("characters.subtitle", { n: String(examChars.length), exam: EXAM_DISPLAY_NAMES[settings.activeExamType] })}
      />
      ...
      <div className="mb-5 flex flex-wrap items-end gap-3">
        ...
        <Select aria-label={t("characters.radicalFilter")} value={radical} onChange={(e) => setRadical(e.target.value)} className="w-40">
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={<Search className="h-8 w-8" />} title={t("characters.noResults")} description={t("characters.noResultsDesc")} />
      ) : (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {visible.map((c) => {
            ...
            return (
              ...
              >
                <span className="hanzi block text-5xl" lang="zh-CN">{c.char}</span>
                <PinyinText pinyin={c.pinyin} zhuyin={c.zhuyin} className="mt-2 block text-sm font-medium" />
                {showsTranslation(settings.displayMode.mode) && (
                  <span className="block text-xs text-ink-faint">{c.meaning}</span>
                )}
                <span className="mt-1.5 block text-xs text-ink-faint">{t("characters.strokes", { n: String(c.strokes), radical: c.radical })}</span>
              </Link>
            );
          })}
        </div>
