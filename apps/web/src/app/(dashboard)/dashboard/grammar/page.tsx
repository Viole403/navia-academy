...
export default function GrammarPage() {
  ...
  const examGrammar = useMemo(
    () =>
      GRAMMAR_POINTS.filter((g) =>
        Boolean(g.examMappings?.[settings.activeExamType])
      ),
  ...
  const filtered = useMemo(() => {
    ...
    return examGrammar.filter((g) => {
      if (level !== "all") {
        const gLevel = String(
          g.examMappings?.[settings.activeExamType] ?? g.level ?? g.hsk
        )
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className="mb-5 flex flex-wrap items-end gap-3">
        ...
        <Select
          aria-label={t("grammar.levelFilter")}
          value={level}
        ...
        <Select
          aria-label={t("grammar.difficultyFilter")}
          value={difficulty}
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <ExamBadge
                    hsk={g.hsk}
                    examMappings={g.examMappings}
                    currentExam={settings.activeExamType}
                  />
                </div>
                <p className="hanzi mt-1 text-sm text-accent" lang="zh-CN">
                  {g.pattern}
                </p>
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span
                    ...
                    aria-label={t("grammar.difficultyOf", {
                      n: String(g.difficulty),
                    })}
                  >
                    {"◆".repeat(g.difficulty)}
