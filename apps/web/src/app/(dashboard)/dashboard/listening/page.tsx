  ...
  for (const [i, w] of chosen.entries()) {
    if (kind === "word") {
      ...
      drills.push({
        ...
        options: shuffle([w, ...distractors]).map((x) => ({
          id: x.id,
          label: x.hanzi ?? x.text ?? "",
          sublabel: x.translation,
        })),
        correct: w.id,
        skill: "listening",
      ...
      const mono = pool.filter(
        (x) => (x.tones ?? []).length === 1 && (x.tones ?? [])[0] !== 0
      )
      ...
      drills.push({
        ...
        options: [1, 2, 3, 4].map((tn) => ({
          id: `t${tn}`,
          label: t(TONE_KEYS[tn]),
        })),
        correct: `t${tone}`,
        skill: "listening",
        hsk: (word.level ?? word.hsk ?? 1) as HskLevel,
        explanation: t("listening.explanation", {
          hanzi: word.hanzi ?? word.text,
      ...
      if (!ex) continue
      const others = sample(
        pool.filter((x) => x.id !== w.id && x.examples[0]),
        3
      ).map((x) => x.examples[0])
      drills.push({
        id: `ls-${i}-${w.id}`,
        ...
        options: shuffle([ex, ...others]).map((x, j) => ({
          id: `o${j}-${x.hanzi ?? x.text ?? ""}`,
          label: x.translation,
        })),
        correct: `?`,
        skill: "listening",
...
export default function ListeningPage() {
  ...
  const listeningMinutes = useMemo(
    () =>
      Object.values(progress.sessions).reduce(
        (a, l) => a + (l.skills.listening ?? 0),
        0
      ),
    [progress.sessions]
  )
  ...
  const poolFor = (exam: typeof settings.activeExamType) =>
    vocabulary.filter(
      (w) =>
        level === "all" ||
        String(w.examMappings?.[exam] ?? w.level ?? w.hsk) === level
    )
            ...
            <Button
              onClick={() => {
                setSession(
                  buildDrills(
                    kind,
                    poolFor(settings.activeExamType),
                    Number(length),
                    t
                  )
                )
    ...
    return (
      <div className="mx-auto max-w-xl">
        ...
        <ProgressBar
          ...
          max={session.length}
          className="mb-6"
          label={t("listening.sessionProgress")}
        />
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
        ...
        />
        <StatCard
          label={t("listening.tip")}
          value={t("listening.slowAudio")}
          sub={t("listening.slowAudioSub")}
        />
      </div>
      ...
      <Card className="mt-6 p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <Select
            label={t("listening.exerciseType")}
            value={kind}
          ...
          >
            ...
            <option value="12">
              {t("listening.durationOpt", { n: "12" })}
            </option>
            <option value="20">
              {t("listening.durationOpt", { n: "20" })}
            </option>
          </Select>
        </div>
        <Button
