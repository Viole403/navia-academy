...
export default function WritingPage() {
  ...
  const charSet = useMemo(
    () => charSetIds.map((id) => CHARACTERS.find((c) => c.id === id)).filter((c): c is HanziChar => Boolean(c)),
  ...
  if (orderExercises.length === 0 || (charScript && tab === "copiar" && charSet.length === 0)) {
    return (
      <div className="animate-fade-up">
        <SectionHeader  title={t("nav.writing")} subtitle={t("writing.subtitle")} />
    ...
    <div className="animate-fade-up">
      ...
      <Tabs
        tabs={[
          ...
          ...(charScript ? [{ id: "copiar", label: t("writing.copyChars") }] : []),
      ...
      {tab === "ordenar" && (
        <div className="mx-auto max-w-xl" role="tabpanel" id="writing-tabs-panel-ordenar" aria-labelledby="writing-tabs-tab-ordenar" tabIndex={0}>
          <Card className="p-6" key={orderExercises[orderIndex % orderExercises.length].id + String(orderIndex)}>
            <ExercisePlayer
          ...
          {orderDone && (
            <Button className="mt-4 w-full" onClick={() => { setOrderIndex((i) => i + 1); setOrderDone(false); }}>
      ...
      {tab === "copiar" && (
        <div role="tabpanel" id="writing-tabs-panel-copiar" aria-labelledby="writing-tabs-tab-copiar" tabIndex={0}>
          <div className="mb-4 flex items-center justify-between">
            <p className="text-sm text-ink-soft">{t("writing.practiceGrid")}</p>
            <Button variant="outline" size="sm" onClick={() => setCharSetIds(sample(CHARACTERS, 3).map((c) => c.id))}>
            ...
            {charSet.map((c) => (
              <div key={c.id} className="text-center">
                <p className="mb-2 text-sm text-ink-soft">
                  <Link href={`/dashboard/characters/${c.id}`} className="hanzi text-lg text-accent hover:underline" lang={ttsLocaleFor(settings.language)}>{c.char}</Link>{" "}
                  {c.pinyin} · {c.meaning}
                </p>
                <HanziPractice
                  char={c.char}
      ...
      {tab === "redactar" && (
        <div className="mx-auto max-w-xl" role="tabpanel" id="writing-tabs-panel-redactar" aria-labelledby="writing-tabs-tab-redactar" tabIndex={0}>
          <Card className="p-6">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">{t("writing.prompt")}</p>
                <p className="mt-1 font-medium">{prompts[promptIndex % prompts.length]}</p>
              </div>
              <Button variant="ghost" size="sm" onClick={() => { setPromptIndex((i) => i + 1); setSubmitted(false); setText(""); }}>
            ...
            {!submitted ? (
              <Button className="mt-4" disabled={text.trim().length < 4} onClick={() => {
                setSubmitted(true);
                progress.setNote(`writing-${Date.now()}`, `${prompts[promptIndex % prompts.length]}\n---\n${text}`);
                ...
                <ul className="mt-2 space-y-1.5 text-sm text-ink-soft">
                  {rubric.map((r, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <input type="checkbox" id={`rub-${i}`} className="mt-1 h-3.5 w-3.5 accent-[var(--accent)]" />
                      <label htmlFor={`rub-${i}`} className="cursor-pointer">{r}</label>
                    </li>
                  ))}
                </ul>
                <p className="mt-3 text-xs text-ink-faint">{t("writing.rubricGuide")}</p>
              </div>
            )}
          </Card>
        </div>
