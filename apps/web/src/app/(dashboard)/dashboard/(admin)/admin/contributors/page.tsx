...
export default function AdminContributorsPage() {
  ...
  const softDelete = async (id: string) => {
    await authFetch(`${API_BASE_URL}/api/v1/contributors/${id}`, { method: "DELETE" });
    fetchData();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 mb-6">
        <h1 className="font-display text-2xl font-bold text-ink">Contributors</h1>
          ...
          {(["all", "active", "inactive"] as const).map((f) => (
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-medium border transition-colors cursor-pointer ${
                filter === f ? "bg-accent text-accent-ink border-accent" : "bg-sunken text-ink-soft border-line hover:bg-hover"
              }`}
            >
            ...
            <tbody className="divide-y divide-line">
              {filtered.map((c) => (
                <tr key={c.id} className="hover:bg-sunken/50">
                  ...
                  <td className="px-4 py-3 text-ink-soft">{c.contributions?.join(", ") || "-"}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      ...
                      <button
                        ...
                        title="Toggle active"
                      >
                        <Plus className={`h-4 w-4 ${!c.is_active ? "rotate-45" : ""}`} />
                      </button>
                      <button
      ...
      <Modal open={!!edit} onClose={() => setEdit(null)} title="Edit Contributor">
        {edit && <EditForm contributor={edit} onClose={() => { setEdit(null); fetchData(); }} />}
      </Modal>
    </div>
  );
}

function EditForm({ contributor, onClose }: { contributor: Contributor; onClose: () => void }) {
  const { t } = useTranslation();
  const [name, setName] = useState(contributor.name);
  const [bio, setBio] = useState(contributor.bio ?? "");
  const [languageLevel, setLanguageLevel] = useState(contributor.mandarin_level ?? "");
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-ink-soft">Language Level</label>
        <input
          value={languageLevel}
