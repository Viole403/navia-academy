...
export default function AdminUsersPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-bold text-ink mb-1">{t("admin.users.title")}</h1>
      <p className="mb-6 text-sm text-ink-faint">{t("admin.users.hint")}</p>

      <form onSubmit={handleCreate} className="mb-8 rounded-xl border border-line bg-sunken p-5 space-y-4">
        <h2 className="font-display text-base font-semibold text-ink">{t("admin.users.createTitle")}</h2>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          {created && <span className="text-sm text-success">{t("admin.users.success")}</span>}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-sunken text-left text-xs font-medium uppercase tracking-wide text-ink-faint">
                ...
                <th className="px-4 py-3 text-right">{t("admin.users.changeRole")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-line">
              {users.map((u) => (
                <tr key={u.id} className="bg-bg hover:bg-sunken/60">
                  <td className="px-4 py-3 text-ink">{u.name || <span className="text-ink-faint">—</span>}</td>
                  ...
                  <td className="px-4 py-3 text-xs text-ink-faint">{new Date(u.created_at).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-right">
                    <select
                      value={u.role}
                      onChange={(e) => changeRole(u.id, e.target.value)}
                      className="cursor-pointer rounded-md border border-line bg-raised px-2.5 py-1.5 text-xs text-ink outline-none transition-colors hover:border-line-strong"
