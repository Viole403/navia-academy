...
export default function AdminPage() {
  ...
  const rows = useMemo(
    () => datasets[tab].data as unknown as Record<string, unknown>[],
    [datasets, tab]
  )
  ...
  function exportDataset() {
    const blob = new Blob([JSON.stringify(datasets[tab].data, null, 2)], {
  ...
  return (
    <div className="animate-fade-up">
      ...
      <Tabs
        tabs={Object.entries(datasets).map(([id, d]) => ({
          id,
          label: `${t(d.labelKey)} (${d.data.length})`,
        }))}
        active={tab}
              ...
              {rows.slice(0, 100).map((row, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="hanzi px-4 py-2" lang="zh-CN">
                      {title}
                    </td>
                    ...
                    <td className="px-4 py-2">
                      <Badge tone="success">{t("admin.published")}</Badge>
                    </td>
                  </tr>
                )
              })}
