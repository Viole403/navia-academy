...
import { AreaChart, Area, LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, type TooltipContentProps } from "recharts";
...
import {
  Clock, TrendingUp, CheckCircle, Trophy, BookOpen, Plus, X, Search,
  ChevronLeft, ChevronRight, XCircle, ChevronUp, ChevronDown, Flame, ArrowRight, AlertTriangle,
} from "lucide-react";
...
function renderTooltip(props: TooltipContentProps) {
  ...
  const d = payload[0].payload as { label?: string; fullDate?: string; score?: number; passed?: boolean };
  return (
    <div className="rounded-[var(--radius)] border border-line bg-raised px-3 py-2 text-xs">
      {payload.length > 1 ? (
        ...
        <div>
          ...
          <p className={cn("font-semibold tabular-nums", d.passed ? "text-success" : "text-danger")}>{d.score}%</p>
        </div>
      )}
    </div>
  );
...
function SortHeader({ field, label, active, order, onSort, align = "left" }: SortHeaderProps) {
  return (
    <th
      scope="col"
    ...
    >
      <button
        ...
        className="inline-flex cursor-pointer select-none items-center gap-1 font-medium transition-colors hover:text-ink"
      >
        {label}
        {active ? (
...
function ProgressSkeleton() {
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl">
      <div className={cn(shimmer, "h-8 w-40 mb-2")} />
      <div className={cn(shimmer, "h-4 w-72 mb-6")} />
      <div className="lg:grid lg:grid-cols-[1fr_280px] lg:gap-8">
        <div>
          <div className={cn(shimmer, "h-[220px] w-full mb-6")} />
          <Card className="p-0">
...
function ProgressContent() {
  ...
  const langExamTypes = useMemo(
    () => (languageInfo(settingsLang).examTypes.filter((et) => ALL_EXAM_TYPES.includes(et as ExamType)) ?? []) as ExamType[],
    [settingsLang],
  );
  ...
  const selectedTypes = useMemo(() => {
    ...
    return new Set(raw.split(",").filter((t): t is ExamType => langExamTypes.includes(t as ExamType)));
  ...
  const navigate = useCallback((params: Record<string, string | null | undefined>) => {
    const next = new URLSearchParams(searchParams.toString());
    let resetPage = false;

    for (const [key, value] of Object.entries(params)) {
      if (value === undefined) continue;
      const omit = value === null || value === "" || value === "all" || (key === "page" && (value === "1" || value === null));
    ...
    router.replace(`/dashboard/progress${qs ? `?${qs}` : ""}`, { scroll: false });
  }, [searchParams, router]);
  ...
  const { examAttempts, unattemptedExamTypes } = useMemo(() => {
    ...
    const attempted = new Set(
      examAttempts.map((a) => parseAttemptId(a.assessmentId).examType as ExamType).filter(Boolean)
    );
  ...
  const stat = useMemo(() => {
    ...
    const avgScore = total > 0 ? Math.round(examAttempts.reduce((s, a) => s + a.score, 0) / total) : 0;
    const totalTime = examAttempts.reduce((s, a) => s + a.timeSec, 0);
    const totalMinutes = Object.values(sessions).reduce((s, ses) => s + (ses as { minutes: number }).minutes, 0);
  ...
  const { aggregateChartData, filteredChartData } = useMemo(() => {
    const agg = !hasAttempts ? [] : [...examAttempts]
  ...
  const byExam = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([type, items]) => {
      ...
      return { type: type as ExamType, passed: passedCount, total: items.length, avg, best };
  ...
  const processedAttempts = useMemo(() => {
    ...
    if (urlQ) {
      ...
      result = result.filter((a) => attemptLabel(a).toLowerCase().includes(lower));
    }
    ...
    result.sort((a, b) => {
      ...
      switch (urlSort) {
        ...
        case "result": return dir * ((isPassed(a) ? 1 : 0) - (isPassed(b) ? 1 : 0));
        default: return dir * (bDate - aDate);
      }
    });
    return result;
  }, [examAttempts, selectedTypes, hasActiveFilter, urlQ, urlSort, urlOrder, attemptLabel]);
  ...
  const pageAttempts = useMemo(
    () => processedAttempts.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE),
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl">
      <SectionHeader title={t("nav.progress")} subtitle={t("progress.subtitle")} />

      {/* ── Stat cards (mobile/tablet only, <lg) ── */}
      {hasAttempts && (
        <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4 lg:hidden">
          <StatCard label={t("progress.examsTaken")} value={stat.total} icon={<BookOpen className="h-4 w-4" />} />
          <StatCard label={t("progress.passed")} value={stat.passed} icon={<CheckCircle className="h-4 w-4" />} />
          <StatCard label={t("progress.avgScore")} value={stat.total > 0 ? `${stat.avgScore}%` : "-"} icon={<TrendingUp className="h-4 w-4" />} />
          <StatCard label={t("progress.studyTime")} value={`${Math.floor((stat.totalMinutes + Math.floor(stat.totalTime / 60)) / 60)}h`} icon={<Clock className="h-4 w-4" />} />
          <StatCard label={t("dashboard.streak")} value={streak} icon={<Flame className="h-4 w-4" />} />
          <StatCard label={t("progress.bestStreak")} value={bestStreak} icon={<Trophy className="h-4 w-4" />} />
        </div>
      )}
      ...
      {hasAttempts ? (
            ...
            <div className="space-y-10">
              ...
              <section>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-display text-lg font-semibold">{t("progress.scoreTrend")}</h2>
                  <div className="hidden sm:flex items-baseline gap-1.5 text-sm">
                    <span className="text-ink-soft">{t("progress.average")}</span>
                    <span className={cn(
                      "text-xl font-bold",
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    {hasActiveFilter ? (
                      <LineChart margin={{ top: 4, right: 4, bottom: 0, left: -16 }}>
                        ...
                        <XAxis dataKey="date" tick={{ fontSize: 11, fill: "var(--ink-faint)" }} axisLine={false} tickLine={false} allowDuplicatedCategory={false} />
                        <YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: "var(--ink-faint)" }} axisLine={false} tickLine={false} />
                        ...
                        {filteredChartData.map((series) => (
                          <Line
                            ...
                            activeDot={{ r: 5, fill: examColor(series.type), stroke: "var(--bg-raised)", strokeWidth: 2 }}
                            connectNulls={false}
                            name={examConfig.displayNames[series.type]}
                          />
                        ))}
                      </LineChart>
                    ) : (
                      <AreaChart data={aggregateChartData} margin={{ top: 4, right: 4, bottom: 0, left: -16 }}>
                        <defs>
                        ...
                        <YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: "var(--ink-faint)" }} axisLine={false} tickLine={false} />
                        <Tooltip content={renderTooltip} />
                        <Area type="monotone" dataKey="score" stroke="var(--accent)" strokeWidth={2} fill="url(#scoreGrad)" dot={{ r: 3.5, fill: "var(--accent)", strokeWidth: 0 }} activeDot={{ r: 5, fill: "var(--accent)", stroke: "var(--bg-raised)", strokeWidth: 2 }} />
                      </AreaChart>
                    )}
                  </ResponsiveContainer>
                </Card>
              ...
              <section>
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                  <h2 className="font-display text-lg font-semibold">
                    ...
                    <span className="text-sm font-normal text-ink-soft ml-2">({t("progress.total", { n: String(totalCount) })})</span>
                  </h2>
                  <div className="relative h-8 w-56">
                    ...
                    <input
                      ...
                      className="h-full w-full rounded-md border bg-raised pl-7 pr-7 text-xs text-ink placeholder:text-ink-faint outline-none focus:border-accent transition-colors"
                    />
                    {searchInput && (
                      <button
                        onClick={() => { setSearchInput(""); navigate({ q: null }); }}
                        className="absolute right-1.5 top-1/2 -translate-y-1/2 h-4 w-4 flex items-center justify-center rounded hover:bg-hover transition-colors"
                      >
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b text-left text-ink-soft [&_th]:sticky [&_th]:top-0 [&_th]:bg-sunken">
                            <th className="px-4 py-3 font-medium">{t("progress.exam")}</th>
                            <SortHeader field="date" label={t("progress.date")} active={urlSort === "date"} order={urlOrder} onSort={handleSort} />
                            <SortHeader field="score" label={t("progress.score")} active={urlSort === "score"} order={urlOrder} onSort={handleSort} align="right" />
                            <th className="px-4 py-3 font-medium text-right">{t("progress.time")}</th>
                            <SortHeader field="result" label={t("progress.result")} active={urlSort === "result"} order={urlOrder} onSort={handleSort} />
                        ...
                        <tbody>
                          {pageAttempts.map((a) => {
                            ...
                            return (
                              <tr
                                key={a.id}
                                className="border-b last:border-0 hover:bg-hover/50 transition-colors"
                              >
                                <td className="px-4 py-3">
                                  <span className="font-medium">{examType?.toUpperCase()}</span>
                                  {level && <span className="text-ink-soft ml-1">{level}</span>}
                                </td>
                                <td className="px-4 py-3 text-ink-soft whitespace-nowrap">
                                  {formatDate(a.finishedAt ?? a.startedAt)}
                                </td>
                                <td className="px-4 py-3 text-right">
                                  <span className={cn("font-medium tabular-nums", passed ? "text-success" : "text-danger")}>{a.score}%</span>
                                  <span className="text-ink-soft text-xs ml-1">({a.correct}/{a.total})</span>
                                </td>
                                <td className="px-4 py-3 text-ink-soft whitespace-nowrap text-right tabular-nums">{formatTime(a.timeSec)}</td>
                                <td className="px-4 py-3">
                                  <Badge tone={passed ? "success" : "danger"}>
                                    <span className="inline-flex items-center gap-1">
                                      {passed ? <CheckCircle className="h-3 w-3" /> : <XCircle className="h-3 w-3" />}
                                      {passed ? t("progress.passed") : t("progress.failed")}
                                    </span>
                                  </Badge>
                                </td>
                                <td className="px-4 py-3">
                                  <button
                                    onClick={() => router.push(`/dashboard/exam/${examType}/${level}`)}
                                    className="text-accent hover:text-accent-strong text-xs font-medium"
                                  >
                                    {t("progress.retake")}
                                  </button>
                                </td>
            ...
            <aside className="hidden lg:block space-y-3">
                ...
                <Card className="p-4">
                  <div className="grid grid-cols-2 gap-2">
                    <StatTile label={t("progress.examsTaken")} value={stat.total} />
                    <StatTile label={t("progress.passed")} value={stat.passed} />
                    <StatTile label={t("progress.passRate")} value={stat.total > 0 ? `${Math.round((stat.passed / stat.total) * 100)}%` : "-"} />
                    <StatTile label={t("progress.studyTime")} value={`${Math.floor((stat.totalMinutes + Math.floor(stat.totalTime / 60)) / 60)}h`} />
                    <StatTile label={t("dashboard.streak")} value={t("progress.days", { n: String(streak) })} />
                    <StatTile label={t("progress.bestStreak")} value={t("progress.days", { n: String(bestStreak) })} />
                  </div>
                </Card>
                ...
                {byExam.length > 0 && (
                  <Card className="p-4">
                    <h3 className="text-xs font-semibold text-ink-soft mb-2.5 uppercase tracking-wider">{t("progress.performance")}</h3>
                    <div className="space-y-0.5">
                      {hasActiveFilter && (
                        <button
                          onClick={clearFilter}
                          className="flex items-center gap-1 w-full py-1 text-left text-xs text-accent hover:underline"
                        >
                      ...
                      {byExam.map(({ type, passed, total, avg }) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "flex items-center justify-between w-full rounded-lg px-2 -mx-2 py-2 text-left transition-colors",
                              active ? "opacity-100" : "opacity-40 hover:opacity-70",
                              "hover:bg-hover/60"
                            )}
                          >
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span className="inline-block h-1.5 w-1.5 rounded-full shrink-0" style={{ background: examColor(type) }} />
                              <span className={cn("text-xs truncate", active ? "font-medium" : "font-normal")}>
                                {examConfig.displayNames[type]}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <span className={cn("text-xs tabular-nums", passed > 0 ? "text-success" : "text-ink-soft")}>
                                {passed}/{total}
                              </span>
                              <span className="text-xs font-semibold tabular-nums w-8 text-right">{avg}%</span>
                            </div>
                          </button>
                        );
                      })}
                ...
                {unattemptedExamTypes.length > 0 && (
                  <Card className="p-4">
                    <h3 className="text-xs font-semibold text-ink-soft mb-2.5 uppercase tracking-wider">{t("progress.exploreOtherExams")}</h3>
                    <div className="flex flex-wrap gap-1.5">
                      {unattemptedExamTypes.map((t) => (
                        <button
                          key={t}
                          onClick={() => router.push(`/dashboard/exam/${t}`)}
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-medium border border-dashed border-line-strong text-ink-soft hover:text-ink hover:border-accent transition-colors"
                        >
                ...
                {weakAreas.length > 0 && (
                  <Card className="p-4">
                    <h3 className="text-xs font-semibold text-ink-soft mb-2.5 uppercase tracking-wider">{t("progress.areasToImprove")}</h3>
                    <div className="space-y-1.5">
                      {weakAreas.slice(0, 3).map((area) => (
                        <button
                          key={area.label}
                          onClick={() => router.push(area.href)}
                          className="flex items-center gap-2 w-full rounded-lg bg-sunken px-3 py-2 text-left hover:bg-hover/70 transition-colors cursor-pointer group"
                        >
                          <AlertTriangle className="h-3.5 w-3.5 text-warn shrink-0" />
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium truncate">{area.label}</p>
                            <div className="h-1 rounded-full bg-line mt-1.5">
                              <div
                                className={cn("h-1 rounded-full transition-all", area.score < 50 ? "bg-danger" : "bg-warn")}
                                style={{ width: `${Math.max(4, area.score)}%` }}
                              />
                            </div>
                          </div>
                          <ArrowRight className="h-3 w-3 text-ink-soft/50 group-hover:text-ink-soft shrink-0 transition-colors" />
                        </button>
                      ))}
                      {weakAreas.length > 3 && (
                        <p className="text-xs text-accent text-center pt-0.5">
                          {t("progress.moreAreas", { n: String(weakAreas.length - 3) })}
                        </p>
                      )}
                    </div>
                  </Card>
                ...
                {/* ── Next Goals ── */}
                <Card className="p-4">
                  <h3 className="text-xs font-semibold text-ink-soft mb-2.5 uppercase tracking-wider">{t("progress.nextGoals")}</h3>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3 rounded-lg bg-sunken px-3 py-2.5">
                      <Flame className="h-5 w-5 text-warn shrink-0" />
                      <div>
                        <p className="text-xs font-medium text-ink-soft">{t("progress.studyStreak")}</p>
                        <p className="text-base font-bold text-warn">{t("progress.days", { n: String(streak) })}</p>
                      </div>
                    </div>
                  </div>
                </Card>
        ...
        <div className="mt-12 flex flex-col items-center text-center">
          <BookOpen className="h-12 w-12 text-ink-soft/40 mb-4" />
          <h2 className="text-xl font-semibold mb-2">{t("progress.emptyTitle")}</h2>
...
function GoalItem({ label, current, target, unit }: { label: string; current: number; target: number; unit: string }) {
  const { t } = useTranslation();
    ...
    <div className="rounded-lg bg-sunken px-3 py-2.5">
      <div className="flex items-center justify-between mb-1">
        ...
        {done ? (
          ...
          <span className="text-xs text-ink-soft">{current}/{target} {unit}</span>
        )}
      </div>
