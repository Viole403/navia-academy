...
import { dueCards, inExamFor, lastDays, learnedCount, nextLesson, todayLog, xpForNextLevel } from "@/lib/derived";
...
export default function DashboardPage() {
  ...
  const due = useMemo(() => {
    const isInExam = inExamFor(settings.activeExamType, VOCABULARY, CHARACTERS, GRAMMAR_POINTS);
  ...
  const week = useMemo(() => lastDays(progress.sessions, 7), [progress.sessions]);
  ...
  const timeKey = hour < 6 ? "night" : hour < 13 ? "morning" : hour < 21 ? "afternoon" : "night";
  const firstName = user?.displayName?.split(" ")[0] ?? t("dashboard.student");
  const goalPct = Math.min(100, ((today?.minutes ?? 0) / settings.dailyGoalMin) * 100);
  const examLabel = EXAM_DISPLAY_NAMES[settings.activeExamType] ?? settings.activeExamType;
  const cheerText =
    ({ zh: "加油！", ja: "頑張って！", en: "Keep going!", de: "Weiter so!" } as Record<string, string>)[settings.language] ??
    "加油！";

  const skillMinutes: [Skill, number][] = Object.entries(
    Object.values(progress.sessions).reduce<Partial<Record<Skill, number>>>((acc, log) => {
  ...
  const chips = [
    ...
    ...(progress.placement
      ? [{ label: GENERAL_LEVEL_LABELS[progress.placement.generalLevel], tone: CHIP_TONES[1] }]
      : []),
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.22em] text-ink-faint">
            {t("dashboard.greeting", { time: t(`dashboard.greeting.${timeKey}`) })},{" "}
            <span className="font-medium text-ink-soft">{firstName}</span>{" "}
            <span className="hanzi ml-1" lang={ttsLocaleFor(settings.language)}>{cheerText}</span>
          </p>
          <h1 className="font-display mt-2 text-display-lg">{t("features.dashboard.title")}</h1>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {chips.map((c) => (
              <span key={c.label} className={cn("bauhaus-chip", c.tone)}>{c.label}</span>
            ))}
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={() => setCustomize(!customize)}>
            {customize ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            ...
            {WIDGETS.map((id) => (
              <button
                ...
                className={cn(
                  ...
                  hidden(id) ? "border-line text-ink-faint" : "border-accent bg-accent-soft text-accent"
                )}
              >
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {stats.map((s) => (
          <Card key={s.label} className="p-4">
            <div className="mb-3 h-1.5 w-8 rounded-[1px] bg-bauhaus-black" aria-hidden>
              <span className={cn("block h-full", s.bar)} />
            </div>
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-ink-faint">{s.label}</p>
              <span className="text-ink-faint">{s.icon}</span>
            </div>
            <p className="font-display mt-2 text-3xl font-bold tracking-tight text-ink">{s.value}</p>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            ...
            <p className="text-xs text-ink-faint">
              {t("dashboard.minutesOfGoal", { done: String(today?.minutes ?? 0), goal: String(settings.dailyGoalMin) })}
            </p>
          </div>
          <ProgressBar className="mt-3" value={goalPct} label={t("dashboard.dailyGoalProgress")} />
          <p className="mt-2 text-xs text-ink-faint">
            {goalPct >= 100 ? t("dashboard.goalMet") : t("dashboard.streakMotivation")}
          </p>
        </Card>
        <Card className="p-5">
          <div className="flex items-center justify-between">
          ...
          <ProgressBar className="mt-3" value={progress.xp} max={nextLevelXp} label={t("dashboard.xpProgress")} />
          <p className="mt-2 text-xs text-ink-faint">{t("dashboard.levelRange", { from: String(level), to: String(level + 1) })}</p>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="p-5">
          <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">{t("dashboard.nextLesson")}</p>
          {lesson ? (
            <>
              <h2 className="font-display mt-2 text-lg font-semibold">{lesson.title}</h2>
              <p className="text-sm text-ink-faint">
                {unitById(lesson.unitId)?.title} · {lesson.durationMin} min · +{lesson.xp} XP
              </p>
              <Link href={`/dashboard/lesson/${lesson.id}`} className="mt-4 inline-block">
                <Button size="sm">
                  {langLessons[lesson.id] ? t("dashboard.continueLesson") : t("dashboard.startLesson")}{" "}
                  <ArrowRight className="h-4 w-4" />
                </Button>
              </Link>
            </>
          ) : (
            <p className="mt-2 text-sm text-ink-soft">{t("dashboard.allDone")}</p>
          )}
        </Card>
        <Card className="p-5">
          <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">{t("dashboard.reviews")}</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="font-display text-3xl font-bold">{due.length}</span>
            <span className="text-sm text-ink-faint">{t("dashboard.itemsToReview")}</span>
          </div>
          <Link href="/dashboard/review" className="mt-4 inline-block">
            <Button size="sm" variant={due.length > 0 ? "primary" : "outline"}>
              <Brain className="h-4 w-4" /> {due.length > 0 ? t("dashboard.reviewNow") : t("dashboard.goToReviews")}
            </Button>
          </Link>
        </Card>
      </div>
      ...
      {!hidden("weekly") && (
        <Card className="p-5">
          ...
          <div className="mt-4 flex h-28 items-end gap-2" role="img" aria-label={t("dashboard.chartAria")}>
            {week.map((d, i) => {
              const max = Math.max(...week.map((x) => x.minutes), settings.dailyGoalMin);
              const h = d.minutes > 0 ? Math.max(8, (d.minutes / max) * 100) : 4;
              const dayName = new Date(d.date + "T12:00:00").toLocaleDateString(locale, { weekday: "short" });
              return (
                <div key={d.date} className="flex flex-1 flex-col items-center gap-1.5">
                  <span className="text-xs text-ink-faint">{d.minutes > 0 ? `${d.minutes}m` : ""}</span>
                  <div
                    className={cn(
                      "w-full max-w-10",
                      d.minutes > 0
                    ...
                    style={{ height: `${h}%` }}
                  />
                  <span className="text-xs uppercase text-ink-faint">{dayName}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {!hidden("skills") && (
          <Card className="p-5">
            ...
            {skillMinutes.length === 0 ? (
              <p className="mt-3 text-sm text-ink-faint">{t("dashboard.skillsEmpty")}</p>
            ) : (
                ...
                {skillMinutes.slice(0, 6).map(([skill, min]) => (
                  <div key={skill}>
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-ink-faint">{formatMinutes(min)}</span>
                    </div>
                    <ProgressBar className="mt-1" value={min} max={skillMinutes[0][1]} />
                  </div>
                ))}
              </div>
            )}
        ...
        {!hidden("tasks") && (
          <Card className="p-5">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium">{t("dashboard.pendingTasks")}</p>
              <Link href="/dashboard/tasks" className="text-xs font-medium text-accent hover:underline">
                {t("dashboard.viewAll")}
              </Link>
            </div>
            {pendingTasks.length === 0 ? (
              ...
              <ul className="mt-3 space-y-2">
                {pendingTasks.slice(0, 4).map((task) => (
                  <li key={task.id} className="flex items-center justify-between rounded-lg border border-line px-3 py-2">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium">{task.title}</p>
                      <p className="text-xs text-ink-faint">
                        {new Date(task.dueDate + "T12:00:00").toLocaleDateString(locale, { day: "numeric", month: "short" })} · {task.estimatedMin} min
                      </p>
                    </div>
      ...
      {!hidden("achievements") && recentAchievements.length > 0 && (
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium">{t("dashboard.recentAchievements")}</p>
            <Link href="/dashboard/achievements" className="text-xs font-medium text-accent hover:underline">
              {t("dashboard.viewAll")}
            </Link>
          </div>
      ...
      <Card className="flex items-center justify-between gap-4 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium">{t("dashboard.recommendation")}</p>
            <p className="text-sm text-ink-soft">
              {due.length > 5
                ? t("dashboard.recReviews", { count: String(due.length) })
                : progress.placement?.weaknesses?.[0]
                  ? t("dashboard.recWeakPoint", { skill: SKILL_LABELS[progress.placement.weaknesses[0]] })
                  : t("dashboard.recSpeaking")}
            </p>
          </div>
        </div>
        <Link href={due.length > 5 ? "/dashboard/review" : "/dashboard/speaking"}>
