...
function friendlyError(err: unknown, t: (key: string) => string): string {
  ...
  if (code === "INVALID_CREDENTIALS" || msg.includes("invalid email or password")) return t("auth.errInvalidCredentials");
  if (code === "TOKEN_EXPIRED" || code === "UNAUTHORIZED") return t("auth.errSessionExpired");
  if (msg.includes("too-many-requests") || msg.includes("rate limit")) return t("auth.errTooManyAttempts");
  if (msg.includes("network") || msg.includes("fetch failed")) return t("auth.errNetwork");
    ...
    <AuthShell title={t("auth.login")} subtitle={t("auth.loginSubtitle")}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        ...
        <div className="flex justify-end">
          <Link href="/recover" className="text-xs font-medium text-accent hover:underline">
      ...
      <p className="mt-4 rounded-lg bg-sunken px-3 py-2 text-xs text-ink-faint">
        {t("auth.demoTitle")} <strong>demo@navia.academy</strong> / <strong>navia-demo</strong>
      </p>
