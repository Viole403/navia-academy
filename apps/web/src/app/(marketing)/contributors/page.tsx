  ...
  return (
    <section className="relative overflow-hidden">
      <div className="absolute inset-0" aria-hidden>
        ...
        <div className="absolute right-[-12%] top-[12%] h-[26rem] w-[26rem] rounded-full bg-[var(--bauhaus-blue)]/20 blur-3xl" />
      ...
      <div className="relative mx-auto max-w-5xl px-4 pb-20 pt-16 sm:px-6 md:pb-28 md:pt-24">
        <Reveal>
          ...
          <p className="mt-5 max-w-xl text-lg leading-relaxed text-ink-soft">{t("contributors.subtitle")}</p>
        </Reveal>

        {contributors === null && !error && (
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="border border-line bg-sunken p-6 shadow-neo animate-pulse">
        ...
        {error && (
          <div className="mt-10 border border-line bg-sunken shadow-neo p-8 text-center">
            <p className="text-sm text-ink-soft">{t("contributors.error")}</p>
            <button
              onClick={fetchContributors}
              className="mt-4 inline-flex items-center justify-center gap-2 rounded-[var(--radius)] border border-line bg-raised px-5 py-2.5 text-sm font-medium text-ink transition-transform hover:-translate-y-0.5 cursor-pointer"
            >
        ...
        {contributors !== null && contributors.length === 0 && (
          <div className="mt-10 border border-line bg-sunken shadow-neo">
            ...
            <div className="flex flex-wrap gap-3 px-6 pb-6">
              <Link
                ...
                className="inline-flex items-center justify-center gap-2 rounded-[var(--radius)] bg-ink px-5 py-2.5 text-sm font-medium text-paper transition-transform hover:-translate-y-0.5"
              >
                {t("contributors.join")}
              </Link>
            </div>
        ...
        {contributors !== null && contributors.length > 0 && (
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {contributors.map((c) => (
              <div key={c.id} className="border border-line bg-sunken p-6 shadow-neo">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-display font-semibold text-ink">{c.name}</h2>
                    {c.mandarin_level && (
                      <p className="text-xs text-ink-soft">{c.mandarin_level}</p>
                    )}
                  </div>
                </div>
                {c.bio && <p className="mt-4 text-sm leading-relaxed text-ink-soft">{c.bio}</p>}
