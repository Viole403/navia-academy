...
export function HanziDisplay({
  ...
  const script = displayMode.script === "traditional" && traditional ? traditional : hanzi;
  const effectiveMode = resolveMode(displayMode, hsk);
  const showTranslation = showsTranslation(effectiveMode);
  const sizeClass =
    hanziSize === "xl" ? "text-3xl" : hanziSize === "lg" ? "text-2xl" : "text-xl";
  ...
  return (
    <span className={`inline-flex flex-wrap items-baseline gap-x-2 gap-y-0.5 ${className}`}>
      ...
      {(pinyin || zhuyin) && <PinyinText pinyin={pinyin ?? ""} zhuyin={zhuyin} mode={effectiveMode} className="text-sm" />}
