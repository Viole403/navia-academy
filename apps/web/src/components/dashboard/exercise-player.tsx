...
export function ExercisePlayer({ exercise, onResult }: Props) {
  ...
  function checkOrder() {
    ...
    const correct =
      ...
      order.join("|") === exercise.correct.join("|")
    onResult(correct)
  }
  ...
  if (exercise.type === "order-words") {
    return (
      <div>
        ...
        {/* Built sentence */}
        <div
          className="mt-4 flex min-h-14 flex-wrap items-center gap-2 rounded-xl border border-dashed border-line-strong bg-sunken/50 p-3"
          aria-label={t("exercise.builtSentence")}
        >
          {order.length === 0 && (
            <span className="text-sm text-ink-faint">
              {t("exercise.tapWordsInOrder")}
            </span>
          )}
          {order.map((id, i) => {
            ...
            return (
              <button
                ...
                className="hanzi cursor-pointer rounded-lg border border-line bg-raised px-3 py-1.5 text-lg hover:border-danger disabled:cursor-default"
                lang="zh-CN"
              >
                {t.label}
              </button>
          ...
          {tokens
            ...
            .map((t) => (
              <button
                ...
                className="hanzi cursor-pointer rounded-lg border border-line bg-raised px-3 py-1.5 text-lg hover:border-accent hover:bg-hover disabled:cursor-default"
                lang="zh-CN"
              >
                {t.label}
              </button>
        ...
        <div className="mt-4 flex items-center gap-3">
          <Button
            size="sm"
            onClick={checkOrder}
            disabled={checked || order.length !== tokens.length}
          >
            {t("exercise.check")}
          </Button>
            ...
            correctAnswer={
              !orderCorrect && Array.isArray(exercise.correct)
                ? exercise.correct
                    .map(
                      (id) =>
                        exercise.tokens?.find((t) => t.id === id)?.label ?? ""
                    )
                    .join(" ")
                : undefined
            }
  ...
  return (
    <div>
      ...
      {exercise.audioText && (
        <div className="mt-4 flex gap-2">
          <button
            ...
            className="flex cursor-pointer items-center gap-2 rounded-xl border border-line bg-raised px-4 py-2.5 text-sm font-medium hover:bg-hover disabled:opacity-50"
          >
          ...
          <button
            onClick={() =>
              play(exercise.audioText!, {
                rate: Math.max(0.5, audioRate - 0.3),
                onLoadingChange: setAudioLoading,
                onError: () => {},
              })
            }
      ...
      <div
        ...
        role="group"
        aria-label={t("exercise.answerOptions")}
      >
        {options.map((o) => {
          ...
          return (
            <button
              ...
              onClick={() => pick(o.id)}
              className={cn(
                "w-full cursor-pointer rounded-xl border px-4 py-3 text-left transition-colors disabled:cursor-default",
            ...
            >
              <span className="flex items-center justify-between">
                <span className="hanzi text-base font-medium" lang="zh-CN">
                  {o.label}
                </span>
                {done && isCorrect && (
      ...
      {done && picked && (
        <ResultNote
          correct={picked === exercise.correct}
          explanation={exercise.explanation}
        />
      )}
    </div>
...
function ResultNote({
  ...
  return (
    <div
      role="status"
      className={cn(
        "animate-brush-in mt-4 rounded-lg border px-4 py-3 text-sm",
        correct
          ? "border-success/40 bg-accent-soft text-success"
          : "border-danger/40 bg-sunken text-danger"
      )}
    >
      ...
      {correctAnswer && (
        <p className="hanzi mt-1 text-ink" lang="zh-CN">
          {t("exercise.answer")}: {correctAnswer}
        </p>
      )}
