...
export function HanziPractice({ char, size = 240, onQuizComplete }: Props) {
    ...
    async function init() {
      ...
      try {
        writerRef.current = HW.create(containerRef.current, char, {
          ...
          height: size,
          padding: 12,
          strokeColor: getComputedStyle(document.documentElement).getPropertyValue("--ink").trim() || "#1a1714",
          outlineColor: getComputedStyle(document.documentElement).getPropertyValue("--grid-line").trim() || "#e5ddca",
          drawingColor: getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#b3382c",
          showCharacter: true,
          showOutline: true,
      ...
      onComplete: (summary) => {
        ...
        setStatus(
          mistakes === 0
            ...
            : t("hanzi.status.completed", { count: String(mistakes), error: mistakes === 1 ? t("hanzi.error") : t("hanzi.errors") })
        );
  ...
  return (
    <div className="inline-flex flex-col items-center gap-3">
      <div className="tianzige rounded-lg" style={{ width: size, height: size }}>
        <div ref={containerRef} className="relative z-10" style={{ width: size, height: size }} />
      </div>
      <p className="max-w-60 text-center text-xs text-ink-faint" role="status">{status}</p>
      <div className="flex flex-wrap justify-center gap-2">
        <Button size="sm" variant={mode === "watch" ? "primary" : "outline"} onClick={animate}>
          <Play className="h-3.5 w-3.5" /> {t('hanzi.strokeOrder')}
        </Button>
        <Button size="sm" variant={mode === "guided" ? "primary" : "outline"} onClick={() => startQuiz(false)}>
          <Pencil className="h-3.5 w-3.5" /> {t('hanzi.guided')}
        </Button>
        <Button size="sm" variant={mode === "memory" ? "primary" : "outline"} onClick={() => startQuiz(true)}>
