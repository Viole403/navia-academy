...
import { EXAM_BADGE_COLORS, EXAM_ABBREVIATIONS, useExamConfig } from "@/lib/exam-definitions";
...
export function ExamBadge({ examType, level, size = "md", showLabel = false, className = "" }: ExamBadgeProps) {
...
export function ExamBadges({ mappings, showEmpty = false, size = "md", className = "" }: ExamBadgesProps) {
...
export function WordExamBadges({ word, showAll = false, size = "md", className = "" }: WordExamBadgesProps) {
...
export function ExamLevelSelector({ examType, selectedLevel, onSelect, className = "" }: ExamLevelSelectorProps) {
  const examDefinition = {
    hsk: { name: "HSK", levels: ["1", "2", "3", "4", "5", "6", "7"] },
    tocfl: { name: "TOCFL", levels: ["Novice 1", "Novice 2", "Level 1", "Level 2", "Level 3", "Level 4", "Level 5"] },
    goethe: { name: "Goethe-Zertifikat", levels: ["A1", "A2", "B1", "B2", "C1", "C2"] },
    jlpt: { name: "JLPT", levels: ["N5", "N4", "N3", "N2", "N1"] },
    toefl: { name: "TOEFL iBT", levels: ["0-30", "31-60", "61-80", "81-100", "101-120"] },
  }[examType];
...
export function ExamFilterBadge({ examType, level, onRemove, className = "" }: ExamFilterBadgeProps) {
  const examConfig = useExamConfig();
  const color = EXAM_BADGE_COLORS[examType];
  const displayName = examConfig.displayNames[examType] || examType;
...
export function ExamCoverageBar({ examType, level, current, total, showNumbers = true, className = "" }: ExamCoverageBarProps) {
  const examConfig = useExamConfig();
  const color = EXAM_BADGE_COLORS[examType];
