...
export function CommandPalette() {
  ...
  const [prevQueryMode, setPrevQueryMode] = useState<[string, typeof mode]>([
    query,
    mode,
  ])
  ...
  const groups = useMemo(() => {
    ...
    if (showAll || queryLower) {
      if (showAll) {
        const matched = PAGES.filter(
          (p) => !queryLower || t(p.labelKey).toLowerCase().includes(queryLower)
        )
        if (matched.length > 0) {
          result.push({
      ...
      const matchedAttempts = attempts
        .filter((a) => {
          ...
          return attemptLabel(examType, level)
            .toLowerCase()
            .includes(queryLower)
        })
        .slice(0, 5)
    ...
    if (!queryLower || result.every((g) => g.items.length === 0)) {
      ...
      if (
        !queryLower ||
        "exam".includes(queryLower) ||
        t("command.keyStart").includes(queryLower)
      ) {
        actions.push({
          id: "action-exam",
  ...
  const handleSelect = useCallback(
    (item: SearchResult) => {
      ...
      if (item.kind === "page") router.push((item as PageResult).href)
        ...
        router.push((item as AttemptResult).href)
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            <div className="overflow-hidden rounded-[var(--radius)] border border-line bg-raised shadow-xl">
              ...
              <div ref={listRef} className="max-h-80 overflow-y-auto p-2">
                ...
                groups.every((g) => g.items.length === 0) ? (
                  ...
                  groups.map((group) => (
                    <div key={group.label}>
                      <p className="px-3 py-1.5 text-xs font-semibold tracking-wider text-ink-soft uppercase">
                        {group.label}
                      </p>
...
export function CommandPaletteTrigger() {
  ...
  return (
    <button
      ...
      className="flex w-full cursor-pointer items-center gap-2.5 rounded-lg border border-dashed border-line-strong px-3 py-1.5 text-sm text-ink-soft transition-colors hover:border-accent hover:text-ink"
    >
