...
import {
  ...
  TypeCycle,
  type TypeCycleItem,
} from "@/components/marketing/type-cycle"
...
function Hero({
  ...
  const vocab = useMemo<
    { text: string; romanization?: string; gloss: string; key: string }[]
  >(() => {
  ...
  const chipColor = (exam: string) =>
    examConfig?.badgeColors?.[exam] ?? "var(--accent)"
  ...
  const levelCycler = useTypeCycler(levelItems, {
    initialIndex: activeExamIndex,
  })
  ...
  const cyclerWordsLabel =
    cyclerWords != null
      ? `${cyclerWords.toLocaleString(locale === "id" ? "id-ID" : "en-US")} ${t("landing.wordsCount")}`
      : null
  ...
  return (
    <section className="relative overflow-hidden">
      <div className="absolute inset-0" aria-hidden>
        <DriftOrb
          seed={0}
          className="top-[-20%] left-[-10%] h-96 w-96 bg-[var(--bauhaus-red)]/20"
        />
        <DriftOrb
          seed={1}
          className="top-[10%] right-[-12%] h-[28rem] w-[28rem] bg-[var(--bauhaus-blue)]/20"
        />
        <DriftOrb
          seed={2}
          className="bottom-[-30%] left-[30%] h-80 w-80 bg-[var(--bauhaus-yellow)]/25"
        />
      ...
      <div className="relative mx-auto max-w-6xl px-4 pt-16 pb-24 sm:px-6 md:pt-24 md:pb-32">
        <div className="grid items-center gap-12 md:grid-cols-[1.05fr_1fr]">
          <div className="relative" style={{ perspective: 1000 }}>
            ...
            <Reveal delay={60}>
              <h1 className="text-display-xl mt-6 font-display leading-[0.98] tracking-tight text-ink">
                {t("hero.title")
                  .split(". ")
                  .map((sentence, i) => (
            ...
            <Reveal delay={180}>
              <div className="mt-8 flex flex-wrap gap-3">
                <Link
                  href="/register"
                  className="inline-flex items-center gap-2 rounded-[var(--radius)] bg-accent px-6 py-3 font-semibold text-accent-ink transition-colors hover:bg-accent-strong"
                >
                  {t("hero.cta")} <ArrowRight className="h-4 w-4" />
                </Link>
                <Link
                  href="#how-it-works"
                  className="inline-flex items-center gap-2 rounded-[var(--radius)] border border-line px-6 py-3 font-semibold text-ink transition-colors hover:bg-sunken"
                >
                  {t("hero.secondaryCta")}
                </Link>
              </div>
              <p
                aria-hidden
                className="mt-5 font-mono text-xs text-ink-faint md:hidden"
              >
          ...
          <div className="relative" style={{ perspective: 1000 }}>
            <Float3D className="relative">
              <div
                className="shadow-neo-lg rounded-2xl border border-line bg-raised p-5"
                style={{ transformStyle: "preserve-3d" }}
              >
                <div className="flex items-center justify-between">
                  <p className="flex items-center gap-2 text-xs font-medium tracking-widest text-ink-faint uppercase">
                    <span
                      aria-hidden
                      className="inline-block h-2 w-2 rounded-full"
                      style={{ background: chipColor(active) }}
                    />
                    {t("landing.heroPanel")}
                  </p>
                ...
                <div
                  ...
                  role="group"
                  aria-label={t("landing.programsTitle")}
                >
                  {examTypes.map((exam) => (
                    <button
                      key={exam}
                      ...
                      className={cn(
                        "inline-flex min-h-11 cursor-pointer items-center rounded-full border px-3 text-xs font-medium transition-all",
                        active === exam
                ...
                <div className="mt-4 space-y-2">
                  {vocab.map((w) => (
                    <div
                      key={w.key}
                      className="flex items-center justify-between gap-2 rounded-xl border border-line bg-sunken/60 px-3 py-2"
                    >
                      ...
                      <button
                        onClick={() => {
                          play(w.key, {
                            onLoadingChange: (l) =>
                              setLoadingKey(l ? w.key : null),
                            onError: () => speak(w.text),
                          })
                        }}
                        disabled={loadingKey === w.key}
                        aria-label={`${w.text} — ${w.gloss}`}
                        className="flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-full bg-accent text-accent-ink transition-colors hover:bg-accent-strong"
                      >
                        {loadingKey === w.key ? (
                          <Loader2
            ...
            <Float3D
              className="absolute -top-8 -left-6 hidden rotate-[-4deg] md:block"
              tilt={12}
            >
              <div
                className="bauhaus-chip shadow-neo px-4 py-2 text-xs font-bold text-white"
                style={{
                  background: levelCycler.item.color ?? chipColor(active),
                  borderColor: "var(--bauhaus-black)",
                }}
              >
            ...
            {showWordsChip && (
              <Float3D
                className="absolute -top-10 -right-6 hidden rotate-[3deg] md:block"
                tilt={12}
              >
            ...
            <Float3D
              className="absolute -right-7 -bottom-9 hidden rotate-[4deg] md:block"
              tilt={12}
            >
            ...
            <Float3D
              className="absolute -bottom-8 -left-5 hidden rotate-[-3deg] md:block"
              tilt={12}
            >
        ...
        <Marquee
          pauseOnHover
          className="mt-16 border-y border-line bg-sunken/40 py-3"
        >
          {examTypes.map((exam) => {
            const lv = examConfig?.definitions?.[exam]?.levels ?? []
            const abbr = examConfig?.abbreviations?.[exam] ?? exam.toUpperCase()
            return (
              <span key={exam} className="flex items-center gap-3 px-6">
                <span
                  ...
                  style={{ background: chipColor(exam) }}
                />
                <span className="font-display text-xs font-bold tracking-widest text-ink-soft uppercase">
...
const FALLBACK_LESSONS: Record<string, number> = {
  zh: 59,
  de: 30,
  en: 13,
  ja: 12,
...
function StatsStrip({
  ...
  const lessons = useMemo(() => {
    if (initialStats) {
      return Object.keys(FALLBACK_LESSONS).reduce(
        (sum, l) =>
          sum +
          ((initialStats as Record<string, number>)[l] ?? FALLBACK_LESSONS[l]),
        0
      )
    }
  ...
  const stats = [
    ...
    {
      value: new Set(Object.values(EXAM_LANGS)).size,
      suffix: "",
      label: t("landing.statLanguages"),
    },
    { value: lessons, suffix: "+", label: t("landing.statLessons") },
    {
      value: placementTotal ?? 84,
      suffix: "+",
      label: t("landing.statPlacement"),
    },
  ]
...
function ProgramCard({ p, i }: { p: ProgramItem; i: number }) {
  const { t } = useTranslation()
  ...
  return (
    <Reveal delay={i * 70} className={cn(i === 0 && "lg:col-span-1")}>
      <Float3D tilt={6} className="h-full">
        <article
          ref={ref}
          className="group hover:shadow-neo-lg focus-visible:shadow-neo-lg relative flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-raised p-6"
          style={{ transform: `translateZ(${(i % 3) * 14}px)` }}
        >
          <span
            aria-hidden
            className="absolute top-0 right-0 h-24 w-24 translate-x-12 -translate-y-12 rounded-full opacity-15 transition-transform duration-500 group-hover:translate-y-[-4px]"
          ...
          <div className="flex items-center justify-between">
            <span
              className="flex h-10 w-10 items-center justify-center rounded-xl font-display text-lg font-bold text-white shadow-[var(--shadow-neo)]"
              style={{ background: p.color }}
            >
              {MONOGRAM[p.type] ?? p.type.toUpperCase().slice(0, 3)}
            </span>
            <span className="text-xs font-medium text-ink-faint">
              {p.words} {t("landing.wordsCount")}
            </span>
          </div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-ink-soft">
            {t("landing.programDesc", { type: p.name })}
          </p>
          ...
          {p.focus.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {p.focus.map((f) => (
                <span
                  key={f}
                  className="rounded-full bg-sunken px-3 py-1 text-xs font-medium text-ink-soft"
                >
                  {f}
                </span>
              ))}
  ...
  return (
    <Reveal delay={350}>
      <Float3D tilt={6} className="h-full">
        <article
          ref={ref}
          className="group hover:shadow-neo-lg focus-visible:shadow-neo-lg relative flex h-full flex-col overflow-hidden rounded-2xl border border-dashed border-line-strong bg-sunken p-6"
        >
          <span
            aria-hidden
            className="absolute top-0 right-0 h-24 w-24 translate-x-12 -translate-y-12 rounded-full opacity-15 transition-transform duration-500 group-hover:translate-y-[-4px]"
          ...
          <div className="flex items-center justify-between">
            <span
              className="flex h-10 w-10 items-center justify-center rounded-xl text-white shadow-[var(--shadow-neo)]"
              style={{ background: "var(--accent)" }}
            >
          ...
          <h3 className="mt-4 font-display text-xl font-bold text-ink">
            {t("landing.placementCardTitle")}
          </h3>
          <p className="mt-2 text-sm leading-relaxed text-ink-soft">
            {t("landing.placementCardDesc")}
          </p>
          <div className="mt-auto pt-4">
            <Link
              href="/register"
  ...
  const programs = useMemo(() => {
    ...
    return (examConfig.types ?? []).map((type) => {
      ...
      const totalWords =
        ...
        (def
          ? Object.values(def.wordCountPerLevel ?? {}).reduce(
...
const LandingSections = dynamic(() =>
  import("./landing-sections").then((m) => m.LandingSections)
)
