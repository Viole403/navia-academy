...
function nestMap<T>(v?: Record<string, T> | Record<string, Record<string, T>>): Record<string, Record<string, T>> {
...
function nestArray(v?: string[] | Record<string, string[]>): Record<string, string[]> {
  if (!v) return {};
  if (Array.isArray(v)) return { zh: v };
  return v as Record<string, string[]>;
}

function normalizeProgressData(data: Partial<ProgressState>): Partial<ProgressState> {
  return {
    ...data,
...
export const useProgress = create<ProgressState>()(
  (set, get) => ({
    ...
    logStudy: (minutes, skill, xp = 0) =>
      set((s) => {
        const today = todayISO();
        const existing = s.sessions[today] ?? { date: today, minutes: 0, xp: 0, skills: {} };
        const log: StudySessionLog = {
          ...existing,
          minutes: existing.minutes + minutes,
          xp: existing.xp + xp,
          skills: { ...existing.skills, [skill]: (existing.skills[skill] ?? 0) + minutes },
        };
        return {
      ...
      set((s) => {
        ...
        return {
          lessons: {
            ...
            [lang]: {
              ...bucket,
              [lessonId]: { ...(bucket[lessonId] ?? { lessonId, completed: false, startedAt: new Date().toISOString() }), step },
            },
          },
        };
      }),
        ...
        return {
          lessons: {
            ...
            [lang]: {
              ...bucket,
              [lessonId]: {
                ...(bucket[lessonId] ?? { lessonId, step: 0, startedAt: new Date().toISOString() }),
                completed: true,
    ...
    ensureCard: (itemId, kind) =>
      set((s) => {
        ...
        return bucket[itemId] ? {} : { srs: { ...s.srs, [lang]: { ...bucket, [itemId]: newCard(itemId, kind) } } };
      }),
    ...
    reviewCard: (itemId, grade) =>
      set((s) => {
        ...
        if (!card) return {};
        return { srs: { ...s.srs, [lang]: { ...bucket, [itemId]: gradeCard(card, grade) } } };
      }),
    ...
    toggleSaved: (wordId) =>
      set((s) => {
        ...
        return {
          savedWordIds: {
            ...s.savedWordIds,
            [lang]: arr.includes(wordId) ? arr.filter((w) => w !== wordId) : [...arr, wordId],
          },
        };
      }),
    ...
    toggleDifficult: (itemId) =>
      set((s) => {
        ...
        return {
          difficultItemIds: {
            ...s.difficultItemIds,
            [lang]: arr.includes(itemId) ? arr.filter((w) => w !== itemId) : [...arr, itemId],
          },
        };
      }),
    ...
    setNote: (itemId, note) =>
      set((s) => {
        const lang = getLearningLanguage();
        return {
          notes: { ...s.notes, [lang]: { ...(s.notes[lang] ?? {}), [itemId]: note } },
        };
      }),
    ...
    unlockAchievements: async () => {
      ...
      const totalMinutes = Object.values(s.sessions).reduce((a, l) => a + l.minutes, 0);
      ...
      const wordsLearned = allSrs.filter((c) => c.kind === "word" && c.mastery >= 30).length;
      const charsLearned = allSrs.filter((c) => c.kind === "character" && c.mastery >= 30).length;
      ...
      const metrics: Record<string, number> = {
        ...
        examTypes: new Set(
          s.attempts.filter((a) => a.score >= 60).map((a) => a.assessmentId.split("-")[0])
        ).size,
      };
...
export async function loadProgressFromServer(): Promise<Partial<ProgressState> | null> {
  try {
    ...
    const body = (await res.json()) as { data?: Partial<ProgressState> | null; success?: boolean };
...
export function lessonsFor(state: ProgressState, lang: LanguageCode): Record<string, LessonProgress> {
  return state.lessons[lang] ?? EMPTY_LESSONS;
}

export function srsFor(state: ProgressState, lang: LanguageCode): Record<string, SrsCard> {
  return state.srs[lang] ?? EMPTY_SRS;
}

export function savedFor(state: ProgressState, lang: LanguageCode): readonly string[] {
  return state.savedWordIds[lang] ?? EMPTY_IDS;
}

export function difficultFor(state: ProgressState, lang: LanguageCode): readonly string[] {
  return state.difficultItemIds[lang] ?? EMPTY_IDS;
}

export function notesFor(state: ProgressState, lang: LanguageCode): Record<string, string> {
