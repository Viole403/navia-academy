...
import type { DisplayMode, DisplayModeMode, ExamType, LanguageCode, ThemeId } from "@/types";
...
export interface SettingsState {
  ...
  set: (patch: Partial<Omit<SettingsState, "set" | "toggleWidget" | "setDisplayMode" | "load" | "hydrated">>) => void;
...
export const DISPLAY_MODES: DisplayModeMode[] = ["hanyu+trans", "zhuyin+trans", "hanyu", "zhuyin", "all", "none"];

/**
 * Display-mode options for pickers: "all" is pinned first, the rest are
 * sorted alphabetically by their localized label.
 */
export function orderedDisplayModes(label: (mode: DisplayModeMode) => string, locale: string): DisplayModeMode[] {
...
export async function loadSettingsFromServer(): Promise<Partial<SettingsState> | null> {
  try {
    ...
    const body = (await res.json()) as { data?: Partial<SettingsState> | null; success?: boolean };
