...
func (r *SupporterRepository) Upsert(ctx context.Context, s *models.Supporter) error {
	err := r.pool.QueryRow(ctx, `
		INSERT INTO supporter (name, avatar_url, platform, amount, message, is_public, external_id, donated_at)
		...
		RETURNING id
	`, s.Name, s.AvatarURL, s.Platform, s.Amount, s.Message, s.IsPublic, s.ExternalID, s.DonatedAt).Scan(&s.ID)
	if err != nil {
		return err
	}
	return nil
...
func (r *SupporterRepository) ListPublic(ctx context.Context, limit, offset int) ([]models.Supporter, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, name, avatar_url, platform, amount, message, is_public, external_id, donated_at, created_at
		FROM supporter
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Name, &s.AvatarURL, &s.Platform, &s.Amount, &s.Message,
			&s.IsPublic, &s.ExternalID, &s.DonatedAt, &s.CreatedAt); err != nil {
			return nil, err
		}
