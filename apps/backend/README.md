...
(root: `bun run backend:typecheck`) + `go build ./cmd/server`.
...
| `CONTENT_EXPORT_TOKEN`                                               | Bearer token app-to-app untuk `GET /content/export` (dipakai `bun run sync-content` milik apps/media). Kosong = endpoint disabled (503) | —                                     |
